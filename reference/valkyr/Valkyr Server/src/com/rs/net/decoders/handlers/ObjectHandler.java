...
import com.rs.game.player.controlers.ConstructionControler;
...
import com.rs.game.player.quests.QuestGuides;
	...
	private static void handleOption1(final Player player, final WorldObject object, final InputStream stream) {
		...
		player.setCoordsEvent(new CoordsEvent(object, new Runnable() {
			...
			public void run() {
				...
				if (objectDef.getName().contains("Banana Tree")) {
					if (player.getInventory().hasFreeSlots()) {
						if (!(object.getId() == 2078)) {
							World.spawnTemporaryObject(new WorldObject(object.getId() + 1, object.getType(), object.getRotation(), object), 60000);
							player.getInventory().addItem(new Item(1963));
							player.sm("You pick the banana from the tree...");
						} else {
							player.sm("This tree doesn't have any bananas on it.");
						}
					} else {
						player.sm("You need more free inventory space.");
					}
					return;
				}
				...
				if (id == 14046) {
					player.getInterfaceManager().openTeleportInterface(player, 100);
					//player.getPackets().sendGameMessage("The teleporter makes an odd sound. I think you should stop using it."); //TODO
					//World.sendGraphics(player, new Graphics(3819), object);
				}
				if (id == 14044) {
					player.sm("Welcome to " + Settings.SERVER_NAME + " Shopping Center.");
					TeleportManager.teleportNormal(player, new WorldTile(2762, 3511, 1), false);
				}
				if(id == 90224){
					player.getActionManager().setAction(new com.rs.game.player.actions.divination.DivineFishing
					...
					if (player.starterstage == 3) {
		                ...
		                for (Player players : World.getPlayers()) {
		                    if (players == null)
		                        continue;
		                    if (player.isPker) {
		                    	players.getPackets().sendGameMessage("<img=7><col=FF0000>All welcome "+ player.getDisplayName() +" to Vuse-RSPS, they are playing as a pker!</col></img>");
		                    } else {
		                    	players.getPackets().sendGameMessage("<img=7><col=FF0000>All welcome "+ player.getDisplayName() +" to Vuse-RSPS, they are using the game mode "+gameMode+"!</col></img>");	
		                    }
		               		 player.starter += 1;
						}
					} else
				...
				if (id == 52760) {
					...
					int y = player.getY();
					if (x == 2222 && y == 3333) {
						player.setNextWorldTile(new WorldTile(2222, 3331, 0));
					}
					if (x == 2222 && y == 3331) {
						player.setNextWorldTile(new WorldTile(2222, 3333, 0));
					}
					if (x == 2217 && y == 3339) {
						player.setNextWorldTile(new WorldTile(2215, 3339, 0));
					}
					if (x == 2215 && y == 3339) {
						player.setNextWorldTile(new WorldTile(2217, 3339, 0));
					}
					if (x == 2222 && y == 3346) {
						player.setNextWorldTile(new WorldTile(2222, 3348, 0));
					}
					if (x == 2222 && y == 3348) {
						player.setNextWorldTile(new WorldTile(2222, 3346, 0));
					}
					if (x == 2228 && y == 3339) {
						player.setNextWorldTile(new WorldTile(2230, 3339, 0));
					}
					if (x == 2230 && y == 3339) {
						player.setNextWorldTile(new WorldTile(2228, 3339, 0));
					}
					if (x == 2222 && y == 3343) {
						player.setNextWorldTile(new WorldTile(2222, 3345, 0));
					}
					if (x == 2222 && y == 3345) {
						player.setNextWorldTile(new WorldTile(2222, 3343, 0));
					}
					if (x == 6299 && y == 6282) {
					...
					if (x == 6297 && y == 6282) {
						player.getPackets().sendGameMessage("Use an Ascension Keystone on this door to open it.");
					}
				}
	...
	if (id == 27896) { // death task portal grim reaper
		...
		return;
			...
			if (id == 40486 || id == 40505 || id == 40534 || id == 40492 || id == 40646
				...
				|| id == 40504) {
				LividFarm.MakePlants(player, object);
            }
			if (id == 40444) {
			...
			// Wilderness course start
			    ...
			    WildernessAgility.exitWildernessCourse(player);
				...
				} else if (id == 9294 && player.getX() == 2880 && player.getY() == 9814) {
					if (player.getSkills().getLevel(Skills.AGILITY) >= 81) {
						player.setNextWorldTile(new WorldTile(2878, 9812, 0));
						player.sm("You jump over the trap.");
					} else {
						player.sm("You jump over the trap, but your not skillful enough!");
						player.setNextWorldTile(new WorldTile(2878, 9812, 0));
						player.applyHit(new Hit(player, player.getHitpoints(), HitLook.REGULAR_DAMAGE));
					}
				} else if (id == 9294 && player.getX() == 2878 && player.getY() == 9812) {
					if (player.getSkills().getLevel(Skills.AGILITY) >= 81) {
					...
					if (player.getSkills().getLevel(Skills.AGILITY) > 60) {
						Magic.pushLeverTeleport(player, new WorldTile(3422, 3551, 1));
					} else
					...
					Magic.pushLeverTeleport(player, new WorldTile(3433, 3538, 1));
				}
				else if ((id == 5282)) {
	...
	WorldTasksManager.schedule(new WorldTask() {
		...
		public void run() {
			if (!secondloop) {
				...
				if (player.isCompletedFightCaves() == true) {
					FightKiln.enterFightKiln(player, false);
					player.sendMessage("You walk into the dark, scorching hot cave.");
				} else {
					player.sm("You can't start the fight kiln till you've completed the fight cave.");
				}
...
player.getPackets().sendGameMessage("You need at least 71 agility to climb this.");
				...
				//crucible
					...
					case "crystal tree shard":
						if (objectDef.containsOption(0, "Chop") && object.getId() == 87536) {
							player.getActionManager().setAction(
									new Woodcutting(object,
											TreeDefinitions.CRYSTAL_1));
						}
						if (objectDef.containsOption(0, "Chop") && object.getId() == 87537) {
							player.getActionManager().setAction(
									new Woodcutting(object,
											TreeDefinitions.CRYSTAL_2));
						}
						if (objectDef.containsOption(0, "Chop") && object.getId() == 87538) {
							player.getActionManager().setAction(
									new Woodcutting(object,
											TreeDefinitions.CRYSTAL_3));
						}
						break;
					...
					case "bamboo gate":
						if (Draynor.isObject(object))
							break;
						if (object.getId() == 25640 || object.getId() == 25638) {
							/*if (player.getY() > 3503)
								player.setNextWorldTile(new WorldTile(player.getX(), player.getY() - 1, player.getPlane()));
							else if (player.getY() <= 3503 && player.getY() > 3501) 
								player.setNextWorldTile(new WorldTile(player.getX(), player.getY() + 1, player.getPlane()));
							break;*/
							player.sm("You can't open this door.");
							break;
						}
						if (objectDef.containsOption(0, "Open"))
						...
						if (objectDef.containsOption(0, "Pray") || objectDef.containsOption(0, "Pray-at")) {
							...
							if (id == 6552)
								player.getDialogueManager().startDialogue(
										"AncientAltar");
							if (id == 13199)
								player.getDialogueManager().startDialogue("AllAltars");
						}
						break;
					}
				}
				if (player.getRights() >= 2 && player.debug) {
					player.getPackets().sendPanelBoxMessage("Object: Click 1 - ID: " + object.getId() + " - X: " + object.getX() + " - Y: " 
							+ object.getY() + " - Z: " + object.getPlane() + " - Type: " + object.getType() + " - Rotation: " 
									+ object.getRotation() + " - ConfigByFile: " + object.getDefinitions().configFileId + ".");
				}
			}
			...
			public void run() {
				...
				if (id == 3509) {
					if (player.getInventory().getFreeSlots() > 0) {
						player.setNextAnimation(new Animation(9064));
				...
				if (player.getRights() >= 2 && player.debug) {
					player.getPackets().sendPanelBoxMessage("Object: Click 2 - ID: " + object.getId() + " - X: " + object.getX() + " - Y: " 
							+ object.getY() + " - Z: " + object.getPlane() + " - Type: " + object.getType() + " - Rotation: " 
									+ object.getRotation() + " - ConfigByFile: " + object.getDefinitions().configFileId + ".");
				}
			}
	...
	private static void handleOption3(final Player player, final WorldObject object) {
		...
		player.setCoordsEvent(new CoordsEvent(object, new Runnable() {
			...
			public void run() {
				...
				if (player.getRights() >= 2 && player.debug) {
					player.getPackets().sendPanelBoxMessage("Object: Click 3 - ID: " + object.getId() + " - X: " + object.getX() + " - Y: " 
							+ object.getY() + " - Z: " + object.getPlane() + " - Type: " + object.getType() + " - Rotation: " 
									+ object.getRotation() + " - ConfigByFile: " + object.getDefinitions().configFileId + ".");
				}
			}
	...
	private static void handleOption4(final Player player, final WorldObject object) {
		...
		player.setCoordsEvent(new CoordsEvent(object, new Runnable() {
			...
			public void run() {
				...
				if (player.getRights() >= 2 && player.debug) {
					player.getPackets().sendPanelBoxMessage("Object: Click 4 - ID: " + object.getId() + " - X: " + object.getX() + " - Y: " 
							+ object.getY() + " - Z: " + object.getPlane() + " - Type: " + object.getType() + " - Rotation: " 
									+ object.getRotation() + " - ConfigByFile: " + object.getDefinitions().configFileId + ".");
				}
			}
	...
	private static void handleOption5(final Player player, final WorldObject object) {
		...
		player.setCoordsEvent(new CoordsEvent(object, new Runnable() {
			...
			//	}
				...
				if (player.getRights() >= 2 && player.debug) {
					player.getPackets().sendPanelBoxMessage("Object: Click 5 - ID: " + object.getId() + " - X: " + object.getX() + " - Y: " 
							+ object.getY() + " - Z: " + object.getPlane() + " - Type: " + object.getType() + " - Rotation: " 
									+ object.getRotation() + " - ConfigByFile: " + object.getDefinitions().configFileId + ".");
				}
			}
	...
	private static void handleOptionExamine(final Player player, final WorldObject object) {
		if (player.getRights() >= 2 && player.debug) {
			player.getPackets().sendPanelBoxMessage("Object: Examine - ID: " + object.getId() + " - X: " + object.getX() + " - Y: " 
					+ object.getY() + " - Z: " + object.getPlane() + " - Type: " + object.getType() + " - Rotation: " 
							+ object.getRotation() + " - ConfigByFile: " + object.getDefinitions().configFileId + ".");
		} else {
	...
	public static void handleItemOnObject(final Player player, final WorldObject object, final int interfaceId, final Item item) {
		...
		player.setCoordsEvent(new CoordsEvent(object, new Runnable() {
			...
			public void run() {
				...
				if (object.getId() == 30543) {
					if (player.getInventory().contains(new Item(553, 1)) && 
							player.getQuestManager().getQuestStage(Quests.THE_RESTLESS_GHOST) == 4 && item.getId() == 553) {
						player.sm("You carefully place the skull back into the coffin...");
						player.setNextAnimation(new Animation(827));
						player.getInventory().deleteItem(new Item(553));
						player.spins += 2;
						player.questPoints++;
						TheRestlessGhost.handleQuestCompleteInterface(player);
						player.getQuestManager().completeQuest(Quests.THE_RESTLESS_GHOST);
					} else {
				...
				case "altar":
					...
					if(bone != null && player.getControlerManager().getControler() instanceof HouseControler) {
				...
				if (itemId == 1438 && object.getId() == 2452) {
					...
					if (!(player.getControlerManager().getControler() instanceof HouseControler)) {
						player.getDialogueManager().startDialogue("SimpleMessage", "You need to be in a player-owned house to repair this item.");
						return;
					}
					...
					case "marble fireplace":
						...
						break;
					case "energy barrier":
						if (World.isAscensions() == false) {
							if (itemId == 28449) {
								player.getInventory().deleteItem(28449, 1);
								player.setNextWorldTile(new WorldTile(6299, 6282, 0));
								player.startAscensionBoss(player);
								World.setAscensionsActive(true);
								player.sm("You've used your key on the door.");
								World.spawnNPC(17151, new WorldTile(6305, 6286, 0), -1, true, true);
							} else if (itemId == 28447) {
								player.getInventory().deleteItem(28447, 1);
								player.setNextWorldTile(new WorldTile(6299, 6282, 0));
								player.startAscensionBoss(player);
								World.setAscensionsActive(true);
								player.sm("You've used your key on the door.");
								World.spawnNPC(17150, new WorldTile(6305, 6286, 0), -1, true, true);
							} else if (itemId == 28451) {
								player.getInventory().deleteItem(28451, 1);
								player.setNextWorldTile(new WorldTile(6299, 6282, 0));
								player.startAscensionBoss(player);
								World.setAscensionsActive(true);
								player.sm("You've used your key on the door.");
								World.spawnNPC(17152, new WorldTile(6305, 6286, 0), -1, true, true);
							} else if (itemId == 28453) {
								player.getInventory().deleteItem(28453, 1);
								player.setNextWorldTile(new WorldTile(6299, 6282, 0));
								player.startAscensionBoss(player);
								World.setAscensionsActive(true);
								player.sm("You've used your key on the door.");
								World.spawnNPC(17153, new WorldTile(6305, 6286, 0), -1, true, true);
							} else if (itemId == 28445) {
								player.getInventory().deleteItem(28445, 1);
								player.setNextWorldTile(new WorldTile(6299, 6282, 0));
								player.startAscensionBoss(player);
								World.setAscensionsActive(true);
								player.sm("You've used your key on the door.");
								World.spawnNPC(17149, new WorldTile(6305, 6286, 0), -1, true, true);
							} else if (itemId == 28455) {
								player.getInventory().deleteItem(28455, 1);
								player.setNextWorldTile(new WorldTile(6299, 6282, 0));
								player.startAscensionBoss(player);
								World.setAscensionsActive(true);
								player.sm("You've used your key on the door.");
								World.spawnNPC(17154, new WorldTile(6305, 6286, 0), -1, true, true);
							}
						} else {
							player.sm("There's currently someone in a fight.");
						}
						break;
					}
					if (Settings.DEBUG) {
