...
import com.rs.game.player.content.OwnerCommands;
...
public final class WorldPacketsDecoder extends Decoder {
	...
	public static void decodeLogicPacket(final Player player, LogicPacket packet) {
		...
		if (packetId == WALKING_PACKET || packetId == MINI_WALKING_PACKET) {
			...
			player.getPackets().sendIComponentText(1314, 30, "Total Boss KC:"); //boss
			player.getPackets().sendIComponentText(1314, 60, "" +other.getBossKills()); //boss
			...
			player.stopAll(false);
			if(player.isInDung()) {
				player.getPackets().sendGameMessage("You can't trade in Dungoneering.");
				return;
			}
            ...
            if (player.isIronMan()) {
				player.getPackets().sendGameMessage("You cannot trade players when you are an Ironman!");
				return;
			}
			if (p2.isIronMan()) {
				player.getPackets().sendGameMessage("You can't trade players that are an Ironman.");
				return;
			}
            ...
            if (player.isIronMan()) {
				player.getPackets().sendGameMessage("You cannot kill when you are an Ironman!");
				return;
			}
			if (p2.isIronMan()) {
				player.getPackets().sendGameMessage("You can't target players they are an Ironman.");
				return;
			}
			...
			case 193:
				...
				case 23:
					if (Magic.checkCombatSpell(player, componentId, 1, false)) {
                        ...
                        if (player.isIronMan()) {
                            player.sm("You can't attack players as an Ironman!");
                            return;
                        }
                        if (p2.isIronMan()) {
                            player.sm("You can't attack a player if they're an Ironman.");
                            return;
                        }
						if (!player.isCanPvp() || !p2.isCanPvp()) {
							player.getPackets()
			...
			if (item == null)
				return;
            if (player.isIronMan() && item.getOwner() != player.getUsername())
                return;
			player.stopAll(false);
			if(forceRun)
				player.setRun(forceRun);
			...
			*/		player.setNextFaceWorldTile(tile);
					...
					if (dropName.contains("pernix") 
						    ...
						    || dropName.contains("abyssal_orb")
						    || dropName.contains("abyssal_wand")
							...
							|| dropName.contains("sirenic_scale")
	...
	public void processPackets(final int packetId, InputStream stream,
		...
		player.afkTimer = Utils.currentTimeMillis() + (15*60*1000);
			...
			if (message.startsWith("@")) {
				OwnerCommands.checkCommand(player, message.replace("@", ""), false, false);
				return;
			}
			...
			if (OwnerCommands.checkCommand(player, command, true, clientCommand)) {
				return;
			}
