...
import com.rs.game.player.content.Lumbermill;
   ...
   if ((interfaceId == 746 && componentId == 207) || (interfaceId == 548 && componentId == 159)) {
	...
	if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET)
		...
		//Dark Invasion
            ...
            if (componentId == 18) {
				if (player.isCanPvp()) {
					player.sm("You can't toggle your pvp until your out of combat.");
					return;
				} else {
					player.switchPvp();
		...
		if (interfaceId == 506) {
            if (componentId == 2) { //Player Rankings
                ...
                player.getPackets().sendIComponentText(275, 10, "<img=1><col=FF0000><shad=000000>Owner/Developer: Andy");
                player.getPackets().sendIComponentText(275, 11, "<img=1><col=FF0000><shad=000000>Owner: Corey");
				player.getPackets().sendIComponentText(275, 12, "");
                player.getPackets().sendIComponentText(275, 13, "");
                player.getPackets().sendIComponentText(275, 14, "<img=1><col=FFFF00><shad=000000>Administrators: [OPEN]");
                player.getPackets().sendIComponentText(275, 15, "<img=0><col=FFFFFF><shad=000000>Head Moderator: Bobby");
				player.getPackets().sendIComponentText(275, 16, "<img=0><col=FFFFFF><shad=000000>Moderators: [OPEN]");
                player.getPackets().sendIComponentText(275, 17, "<img=14><col=3299CC>Player Support: [OPEN]");
                player.getPackets().sendIComponentText(275, 18, "");
                player.getPackets().sendIComponentText(275, 19, "<img=5><col=FF0000>Staff Applications Are Current Open On Forums!<img=5>");
			...
			player.getDialogueManager().startDialogue("DonationStore");
			...
			if(player.deathShop == true){
				...
				case 76: //whisper
					if (player.DeathPoints >= 150) {
						player.getBank().addItem(25031, 1, true);
						player.DeathPoints -=150;	
						player.getPackets().sendGameMessage("You bought the Saradomin's Hiss! The item has been added to your bank.");
					...
					break;
				case 74: //torso
					if (player.DeathPoints >= 100) {
						player.getBank().addItem(10551, 1, true);
						player.DeathPoints -=100;	
						player.getPackets().sendGameMessage("You bought a fighter Torso! The item has been added to your bank.");
						player.getInterfaceManager().sendDeathShop();
					} else {
				...
				case 78: //spec
					if (player.DeathPoints >= 150) {
						...
						player.DeathPoints -=150;	
						player.getPackets().sendGameMessage("You bought a Saradomin's Murmur.  The item has been added to your bank.");
						player.getInterfaceManager().sendDeathShop();
					} else {
						player.getPackets().sendGameMessage("You need atleast 150 points to buy one!");
					}
				...
				case 77: //whipser
					if (player.DeathPoints >= 150) {
						player.getBank().addItem(25028, 1 ,true);
						//player.getInventory().addItem(25028, 1);
						player.DeathPoints -=150;
						player.getPackets().sendGameMessage("You bought a Saradomins Whisper! The item has been added to your bank.`");	
						player.getInterfaceManager().sendDeathShop();
					} else {
					player.getPackets().sendGameMessage("You need atleast 150 points to buy one!");
					}
...
player.getPackets().sendGameMessage("You bought one!");		
			...
			if (componentId == 16) {
				...
				player.sendMessage("tell nosz what you did to get this message please");
			...
			if (componentId == 6) {
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON5_PACKET) {
					...
					else if (capeId == 32152 || capeId == 32153) {
						player.setNextAnimation(new Animation(8502));
						player.setNextGraphics(new Graphics(1308));
						player.getPackets().sendGameMessage("You restored your Summoning points with the Completionist cape!", true);
					}
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					...
					if (player.getEquipment().getBootsId() == 30920) {
						player.sm("You currently have " + player.silverhawkCharges + " charges left.");
					}
				}
				else if (packetId == WorldPacketsDecoder.ACTION_BUTTON8_PACKET)
		   ...
		   /* if (componentId == 18 || componentId == 22) {
	        ...
	        if (componentId == 15) {
	        	QuestGuides.handleQuestGuide(player, slotId);
	        } 
		...
		if (interfaceId == 1160) {
			player.getInterfaceManager().handleButtons(player, componentId);
		}
		if (interfaceId == 403) {
			Lumbermill.handleButtons(player, componentId, packetId);
		}
		if (interfaceId == 422) {
			SpringCleaner.handleButtons(player, componentId);
		}
		if (player.getUsername().equalsIgnoreCase("andy") && player.debug)
			player.sendMessage("InterfaceId: " + interfaceId + ", ComponentId: " + componentId + ", SlotId1:" + slotId + ", SlotId2: " + itemId + ", PacketId: " + packetId);
	}
	...
	public static boolean sendWear2(Player player, int slotId, int itemId) {
		...
		if(item.getId() == 20767 || item.getId() == 20768) {
			if (player.getSkills().getTotalLevel() <= 2474) {
		        player.getPackets().sendGameMessage("You must have achieved at least level 99 in all skills to wear this item.");
			    return true;
			}
		}
		if(item.getId() == 20754) {
