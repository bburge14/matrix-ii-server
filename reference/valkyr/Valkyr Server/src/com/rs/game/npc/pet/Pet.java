...
public final class Pet extends NPC {
	 ...
	 * @param level 
	 */
	public Pet(int id, int itemId, Player owner, WorldTile tile, PetDetails details, int level) {
	...
	public void processNPC() {
		...
		if (pets.isBossPet()) {
			setName(NPCDefinitions.getNPCDefinitions(pet.getBabyNpcId()).getName() + ", <col=00ffcc> Level: " + owner.getPetLevelForId(pets.getBabyNpcId()));
		}
		sendFollow();
	}
	...
	public void growNextStage() {
		...
		finish();
		Pet newPet = new Pet(npcId, itemId, owner, owner, details, level);
