...
public abstract class Familiar extends NPC implements Serializable {
	...
	public String getTimeLeft() {
		return Utils.getFormattedTime(ticks * 60 / 2 + 30);
	}
	...
	public void sendDeath(Entity source) {
		...
		dead = true;
		int id = this.getId();
		if (owner.getFamiliar().isBossPet(id)) {
			if (owner.getInventory().hasFreeSlots()) {
				owner.getInventory().addItem(pouch.getPouchId(), 1);
				owner.sm("<col=ff0000>Your pet has been placed in your inventory.");
			} else {
				owner.getBank().addItem(pouch.getPouchId(), 1, true);
				owner.sm("<col=ff0000>Your pet was sent to your bank.</col>");
			}
		}
		removeFamiliar();
