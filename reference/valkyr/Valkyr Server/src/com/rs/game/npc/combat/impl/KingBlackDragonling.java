	...
	public int attack(NPC npc, Entity target) {
		...
		if (usingSpecial) {// priority over regular attack
			...
			if (distant) {// range hit
				delayHit(npc, 2, target, getMagicHit(npc, getRandomMaxHit(npc, 250, NPCCombatDefinitions.MAGE, target)));
			}
		} else {
			if (distant) {
				...
				damage = getRandomMaxHit(npc, 250, NPCCombatDefinitions.MELEE, target);
				npc.setNextAnimation(new Animation(-1));
