...
public class Bank implements Serializable {
	...
	public void removeItem(Item item) {
		...
		for (int i = 0; i < bankTabs.length; i++) {
			for(int i2 = 0; i2 < bankTabs[i].length; i2++) {
				if(bankTabs[i][i2].getId() == item.getId()) {
					bankTabs[i][i2].setAmount(bankTabs[i][i2].getAmount() - item.getAmount());
				}
			}
		}
		refreshTabs();
	...
	public long getMaxBankSize() {	
		return player.hasPerk("bank space +100") ? (MAX_BANK_SIZE + 100) : MAX_BANK_SIZE;
	}
	...
	public void openDepositBox() {
		if (World.inPvPMode) {
			player.sm("You can't do this while the world is in PvP!");
			return;
		}
	...
	public void openBank() {
		if (player.getOpenedPin() == false && player.bypass == false && player.getSetPin() == true) {
	...
	public boolean hasBankSpace() {
		return getBankSize() < (player.hasPerk("bank space +100") ? MAX_BANK_SIZE + 100 : MAX_BANK_SIZE);
	}
	...
	public void refreshBankSize() {
        ...
        player.getPackets().sendIComponentText(762, 30, player.hasPerk("bank space +100") ? "" + (MAX_BANK_SIZE + 100) : "" + MAX_BANK_SIZE); // free max size 
        player.getPackets().sendIComponentText(762, 31, "" + getBankSize()); // free
        player.getPackets().sendIComponentText(762, 32, player.hasPerk("bank space +100") ? "" + (MAX_BANK_SIZE + 100) : "" + MAX_BANK_SIZE); // free max size 
    }
