...
public class SlayerManager implements Serializable {
	...
	if (interfaceId == BUY_INTERFACE) {
		...
		if (removePoints(35)) {
		    player.getInventory().addItem(560, 2500);
		...
		if (removePoints(35)) {
		    ...
		    player.getPackets().sendGameMessage("Here are your arrows. Use them wisely.");
		}
	    }
	} else if (interfaceId == ABILITIES_INTERFACE) {
	...
	if (socialPlayer != null) {
		...
		socialPlayer.getSkills().addXp(Skills.SLAYER, otherDamageAdmitted / 2);
	    otherSocialCount = socialPlayer.getSlayerManager().getCurrentTaskCount();
	}
	player.getSkills().addXp(Skills.SLAYER, damageAdmitted / 2);
