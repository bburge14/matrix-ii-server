...
public final class PetManager implements Serializable {
	...
	public boolean spawnPet(int itemId, boolean deleteItem) {
		...
		if (npcId > 0) {
			Pet pet = new Pet(npcId, itemId, player, player, details, level);
			this.npcId = npcId;
			this.itemId = itemId;
	...
	public void eat(int foodId, Pet npc) {
		...
		if (pets.isBossPet()) {
			if (player.getPetLevelForId(pets.getBabyNpcId()) < 0)
			player.getInventory().deleteItem(foodId, 1);
			player.getPackets().sendGameMessage("Your pet happily eats the " + ItemDefinitions.getItemDefinitions(foodId).getName() + ".");
			if (pets == Pets.NEXTERMINATOR) {
				player.nexLevel++;
			}
			if (pets == Pets.MALLORY || pets == Pets.GAVIN || pets == Pets.BARRY 
					|| pets == Pets.BILL || pets == Pets.LANA || pets == Pets.PETE || pets == Pets.DAVE || pets == Pets.STEVE) {
				player.arraxiLevel++;
			}
			if (pets == Pets.GENERALAWWDOR) {
				player.bandosLevel++;
			}
			if (pets == Pets.CHICKARRA) {
				player.armadylLevel++;
			}
			if (pets == Pets.KRILTINYROTH) {
				player.krilLevel++;
			}
			if (pets == Pets.COMMANDERMINIANA) {
				player.saraLevel++;
			}
			if (pets == Pets.KINGBLACKDRAGONLING) {
				player.kbdLevel++;
			}
			if (pets == Pets.QUEENBLACKDRAGONLING) {
				player.qbdLevel++;
			}
			if (pets == Pets.MOLLY) {
				player.moleLevel++;
			}
			if (pets == Pets.CORPORALPUPPY) {
				player.corpLevel++;
			}
			if (pets == Pets.KALPHITEGRUBLET) {
				player.kkLevel++;
			}
			if (pets == Pets.KALPHITEGRUBLING) {
				player.kqLevel++;
			}
			if (pets == Pets.VORAGO_SHARD || pets == Pets.VITALIS || pets == Pets.BOMBI) {
				player.voragoLevel++;
			}
			if (pets == Pets.ELLIE) {
				player.chaosLevel++;
			}
			if (pets == Pets.LEGIOGUARTULUS || pets == Pets.LEGIOPRIMULUS || pets == Pets.LEGIOQUINTULUS 
					|| pets == Pets.LEGIOSECUNDULUS || pets == Pets.LEGIOSEXTULUS || pets == Pets.LEGIOTERTIOLUS) {
				player.legioLevel++;
			}
			return;
		}
