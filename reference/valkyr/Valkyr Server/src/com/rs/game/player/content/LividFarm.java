 ...
 */
	 ...
	 * XP Boosting rates
	 */
public static int boostedxp = 500;
	/**
	 * 
	 * @param player
	 */
	 ...
	 * @param player
	 */
	public static void MakePlants(Player player, WorldObject object) {
    ...
    public static void fillBucket(Player player) {
        if (player.getInventory().containsItem(1925, 1)) {
            ...
            player.getInventory().addItem(20933, 1);
            player.sm("You fill the bucket with oil...");
        } else {
            player.sm("You need a bucket to fill.");
        }
    }
	...
	public static void deposit(Player player) {
        if (player.getInventory().containsItem(bunchedplant, 27)) {
            int[] Herbs = { 199, 201, 203, 205, 207, 209, 211, 213, 215, 217, 219, 21626};
            int length = Herbs.length;
			length--;
		    int reward = Utils.getRandom(length);
			player.getBank().addItem(Herbs[reward], Utils.getRandom(10), false);
            ...
            player.getDialogueManager().startDialogue("SimpleNPCMessage", LADY, "I've sent a reward to your bank for helping me!");
	    } else {
	...
	public static void setFarming(Player player) {
		player.lividcraft = false;
		player.sm("You will be gaining now Farming experience only.");
		player.lividmagic = false;
