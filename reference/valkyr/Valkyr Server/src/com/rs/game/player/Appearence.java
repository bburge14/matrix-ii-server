...
public class Appearence implements Serializable {
	...
	public void generateAppearenceData() {
        ...
        if (player.getUsername().equals("") || player.getUsername().equals("corey")) {
			stream.writeByte(player.hasSkull() ? player.getSkullId() : 14);
        } else {
		...
		if (transformedNpcId >= 0) {
			...
			for (int slotId = 0; slotId < player.getEquipment().getItems().getSize(); slotId++) {
				...
				if (player.isAssassin() && isOverrideSlot(slotId)) {
					if (player.getEquipment().getItem(slotId) != null) {
						hash |= 1 << slotFlag;
						stream.writeByte(0x1);
						if (isMale())
							stream.writeBigSmart(ItemConstants.getAssassinModels(slotId, true));
						else 
							stream.writeBigSmart(ItemConstants.getAssassinModels(slotId, false));
					}
				} else {
					if (slotId == Equipment.SLOT_HAT) {
						...
						if (capeId == 20767 || capeId == 20769 || capeId == 20771 || capeId == 32152 || capeId == 32151 || capeId == 32153) {
							ItemDefinitions defs = ItemDefinitions.getItemDefinitions(capeId);
							if ((capeId == 20767 || capeId == 32151) && Arrays.equals( player.getMaxedCapeCustomized(), defs.originalModelColors) || ((capeId == 20769 || capeId == 20771 || capeId == 32152 || capeId == 32153)  && Arrays.equals(player.getCompletionistCapeCustomized(), defs.originalModelColors)))
								continue;
							hash |= 1 << slotFlag;
							stream.writeByte(0x4); // modify 4 model colors
							int[] cape = capeId == 20767 || capeId == 32151 ? player.getMaxedCapeCustomized() : player.getCompletionistCapeCustomized();
							int slots = 0 | 1 << 4 | 2 << 8 | 3 << 12;
							stream.writeShort(slots);
							for (int i = 0; i < 4; i++) {
