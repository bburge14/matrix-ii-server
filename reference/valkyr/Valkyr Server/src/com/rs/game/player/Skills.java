...
import com.rs.game.player.TimeManager;
...
public final class Skills implements Serializable {
	...
	public static int getSkillId(String name) {
		for (int i = 0; i < SKILL_NAME.length; i++) {
			if (name.toLowerCase().equals(SKILL_NAME[i].toLowerCase())) {
				return i;
			}
		}
		return -1;
	...
	public int getLevelForXp(int skill) {
		...
		int points = 0;
		int output = 0;
		for (int lvl = 1; lvl <= (skill == DUNGEONEERING || skill == SLAYER ? 120 : 99); lvl++) {
			points += Math.floor(lvl + 300.0 * Math.pow(2.0, lvl / 7.0));
			output = (int) Math.floor(points / 4);
			if ((output - 1) >= exp) {
				return lvl;
	...
	public void addXp(int skill, double exp) {
		...
		if (player.isXpLocked())
			return;
		if (player.getExpMode() != 0) {	
			if (skill == ATTACK || skill == HITPOINTS || skill == DEFENCE || skill == STRENGTH || skill == RANGE || skill == MAGIC || skill == PRAYER) {
				exp *= player.getExpMode() * 4;
			} else {
				exp *= player.getExpMode();
			}
		}	
