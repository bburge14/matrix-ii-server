...
public class PlayerCombat extends Action {
	...
	public boolean start(Player player) {
		...
		if (weaponName.contains("2h") || weaponName.contains("godsword") || weaponName.contains("noxious scythe") || weaponName.contains("halberd") || weaponName.contains("staff")) {
        ...
        else if (weaponName.contains("flamethrower")){ //flame thrower
            player.getAppearence().setRenderEmote(3242);
        }
		player1 = player;
		if (checkAll(player)) {
			return true;
	...
	private int getRangeCombatDelay(int weaponId, int attackStyle, Player player) {
		int delay = 6;
		if (weaponId != -1) {
			String weaponName = ItemDefinitions.getItemDefinitions(weaponId)
					.getName().toLowerCase();
			String weaponOffhand = ItemDefinitions.getItemDefinitions(player.getEquipment().getShieldId())
					.getName().toLowerCase();
			if (weaponName.contains("crossbow") && weaponOffhand.contains("crossbow"))
				return 3;
			if (weaponName.contains("crossbow") && !weaponOffhand.contains("crossbow"))
				return 5;
			if (weaponName.contains("shortbow") || weaponName.contains("karil's crossbow"))
				delay = 3;
            if (weaponName.contains("flamethrower"))
                delay = 5;
			...
			switch (weaponId) {
				case 31729:
					if (player.getCombatDefinitions().getSpecialAttackPercentage() >= 100) {
						player.getCombatDefinitions().desecreaseSpecialAttack(100);
						player.setNextAnimation(new Animation(18452));
						World.sendProjectile(player, target, 4979, 41, 16, 25, 35, 16, 0);
						delayMagicHit(2, getMagicHit(player, getRandomMagicMaxHit(player, (5 * player.getSkills().getLevel(Skills.MAGIC)) - 150)));
						target.setNextGraphics(new Graphics(4980));
						target.getPoison().makePoisoned(183);
						return 5;
					} else {
						player.sm("You don't have enough special atttack to use this.");
					}
				break;
			
				default:
					player.sm("This weapon doesn't have a special attack!");
					return -1;
			}	
		}
		...
		if (player.getCombatDefinitions().getSpellBook() == 192) {
			...
			case 25: // air strike
				if (player.getEquipment().hasTwoHandedWeapon()) {
					player.setNextAnimation(new Animation(18452));
					player.setNextGraphics(new Graphics(3531));
				} else {
			...
			case 99: // Storm of armadyl //Sonic and Tyler dumped
				...
				int hit = getRandomMagicMaxHit(player, 260 + boost);
				if (hit > 0 && hit < boost)
					hit += boost;
				delayMagicHit(2, getMagicHit(player, hit));
				World.sendProjectile(player, target, 1019, 18, 18, 50, 30, 0, 0);
				return player.getEquipment().getWeaponId() == 21777 ? 4 : 6;
			}
	...
	private int rangeAttack(final Player player) {
		...
		int combatDelay = getRangeCombatDelay(weaponId, attackStyle, player);
		...
		if (player.getCombatDefinitions().isUsingSpecialAttack()) {
			...
			case 31733:
				...
				World.sendProjectile(player, target, 4995, 41, 16, 31, 35, 16, 0);
				final int hit2 = getRandomMaxHit(player, weaponId, attackStyle,
						true, true, 1.5, true);
				...
				WorldTasksManager.schedule(new WorldTask() {
					...
					@Override
					public void run() {
						target.setNextGraphics(new Graphics(4996));
						stop();
					}
				...
				if (hit2 > 0) {
					...
					WorldTasksManager.schedule(new WorldTask() {
						...
						public void run() {
							...
							if (damage > 75) {
								damage -= 75;
								finalTarget.applyHit(new Hit(player, 75,
										HitLook.REGULAR_DAMAGE));
								target.setNextGraphics(new Graphics(3821));
							} else {
								finalTarget.applyHit(new Hit(player, damage,
										HitLook.REGULAR_DAMAGE));
								target.setNextGraphics(new Graphics(3821));
			...
			if (weaponId != -1) {
				...
				}else if (weaponName.contains("flamethrower")) {
                    player.setNextGraphics(new Graphics(5066));
					World.sendProjectile(player, target, 3903, 20, 20, 41, 40, 0, 0);
					World.sendProjectile(player, target, 3903, 20, 20, 41, 40, 30, 0);
					World.sendProjectile(player, target, 3903, 20, 20, 41, 40, 15, 0);
					World.sendProjectile(player, target, 3903, 20, 20, 41, 40, 10, 0);
					World.sendProjectile(player, target, 3903, 20, 20, 41, 40, 5, 0);
					World.sendProjectile(player, target, 3903, 20, 20, 41, 40, 20, 0);
					World.sendProjectile(player, target, 3903, 20, 20, 41, 40, 25, 0);
					
					World.sendProjectile(player, target, 3822, 20, 20, 41, 40, 0, 0);
					World.sendProjectile(player, target, 3822, 20, 20, 41, 40, 30, 0);
					World.sendProjectile(player, target, 3822, 20, 20, 41, 40, 15, 0);
					World.sendProjectile(player, target, 3822, 20, 20, 41, 40, 15, 0);
					World.sendProjectile(player, target, 3822, 20, 20, 41, 40, 10, 0);
					World.sendProjectile(player, target, 3822, 20, 20, 41, 40, 5, 0);
					World.sendProjectile(player, target, 3822, 20, 20, 41, 40, 20, 0);
					World.sendProjectile(player, target, 3822, 20, 20, 41, 40, 25, 0);
					
					World.sendProjectile(player, target, 3743, 20, 20, 41, 40, 0, 0);
					World.sendProjectile(player, target, 3743, 20, 20, 41, 40, 30, 0);
					World.sendProjectile(player, target, 3743, 20, 20, 41, 40, 15, 0);
					World.sendProjectile(player, target, 3743, 20, 20, 41, 40, 15, 0);
					World.sendProjectile(player, target, 3743, 20, 20, 41, 40, 10, 0);
					World.sendProjectile(player, target, 3743, 20, 20, 41, 40, 5, 0);
					World.sendProjectile(player, target, 3743, 20, 20, 41, 40, 20, 0);
					World.sendProjectile(player, target, 3743, 20, 20, 41, 40, 25, 0);
					
					World.sendProjectile(player, target, 3766, 20, 20, 41, 40, 0, 0);
					World.sendProjectile(player, target, 3766, 20, 20, 41, 40, 30, 0);
					World.sendProjectile(player, target, 3766, 20, 20, 41, 40, 15, 0);
					World.sendProjectile(player, target, 3766, 20, 20, 41, 40, 15, 0);
					World.sendProjectile(player, target, 3766, 20, 20, 41, 40, 10, 0);
					World.sendProjectile(player, target, 3766, 20, 20, 41, 40, 5, 0);
					World.sendProjectile(player, target, 3766, 20, 20, 41, 40, 20, 0);
					World.sendProjectile(player, target, 3766, 20, 20, 41, 40, 25, 0);
                    //target.setNextGraphics(new Graphics(3821));
	                //target.setNextGraphics(new Graphics(3263));
	                //target.setNextGraphics(new Graphics(2929));
	                //target.setNextForceTalk(new ForceTalk("ALLAH AKBAR!"));
                    WorldTasksManager.schedule(new WorldTask() {
                        int hit = Utils.getRandom(1000);
    				    int ticks;
    				    @Override
    				    public void run() {
    					    ticks++;
                            if (ticks >= 2) {
                            	//target.setNextForceTalk(new ForceTalk("Help I'm on fire!!"));
                                target.setNextGraphics(new Graphics(3821));
                                //target.setNextGraphics(new Graphics(3904));
            					delayHit(1, weaponId, attackStyle, getRangeHit(player, hit));
                            }
            				if (ticks == 10) {
            					stop();
		...
		if (quantity == -2) {
			...
			if (weaponId != -1) {
				if ((quantity == -3 && Utils.getRandom(10) < 2) || (quantity != -3 && Utils.getRandom(3) > 0)) {
					...
					if (capeId != -1 && ItemDefinitions.getItemDefinitions(capeId).getName().contains("Ava's") || ItemDefinitions.getItemDefinitions(capeId).getName().contains("TokHaar-Kal-Xil") || weaponId == 25202)
						return;
				} else {
			...
			if (Utils.getRandom(3) > 0) {
				int capeId = player.getEquipment().getCapeId();
				if (capeId != -1 && ItemDefinitions.getItemDefinitions(capeId).getName().contains("Ava's") || ItemDefinitions.getItemDefinitions(capeId).getName().contains("TokHaar-Kal-Xil"))
					return;
			} else {
	...
	private int meleeAttack(final Player player) {
		...
		if (weapon.getName().contains("staff") || weapon.getName().contains("wand")) {
			player.sm("You can't attack with this weapon without a spell selected.");
			return -1;
		}
		if (weaponId == -1) {
			Item gloves = player.getEquipment().getItem(Equipment.SLOT_HANDS);
	...
	public static int getWeaponAttackEmote(int weaponId, int attackStyle) {
		...
		if (weaponId != -1) {
			...
			if (weaponName != null && !weaponName.equals("null")) {
				...
				if(shieldName.contains("off-hand") && Utils.random(2) == 0 && !shieldName.contains("cross") && !shieldName.contains("rapier"))
					return 18244;
				...
				if (weaponName.contains("royal crossbow")) {
					return 18468;
				}
	...
	private int getMeleeCombatDelay(Player player, int weaponId) {
		if (weaponId != -1) {
			...
			if (weaponName.equals("zamorakian spear"))
				return 3;
			if (weaponName.contains("rapier") || weaponName.equals("korasi's sword") && weaponOffhand.contains("off-hand"))
				return 2;
			if (weaponName.contains("warhammer") || weaponName.contains("battleaxe") && weaponOffhand.contains("off-hand"))
				return 2;
			if (weaponName.contains("warhammer") || weaponName.contains("battleaxe") && !weaponOffhand.contains("off-hand"))
				return 4;
			if (weaponName.contains("longsword") || weaponName.contains("mace") && weaponOffhand.contains(" longsword") || weaponOffhand.contains("mace"))
				return 3;
			if (weaponName.contains("longsword") || weaponName.contains("mace") && !weaponOffhand.contains(" longsword") && !weaponOffhand.contains("mace"))
				return 5;
			if (weaponName.contains("godsword")
			...
			if (weaponName.contains("greataxe")
					...
					|| weaponName.contains("noxious scythe")
					|| weaponName.equals("thok's sword"))
				return 6;
			// Interval 3.0
			if (weaponName.contains("spear")
	...
	public static final int isRanging(Player player) {
		...
		case 16337:
			...
			case 31737: //Araxtye arrow
			case 16472:// prom arrows
				return 2;
			default:
				return 1;
			}
		...
		case 33318: // Ascension Crossbow
		case 33384: // Ascension Crossbow
        case 31918: // flamethrower
        case 31919: // flamethrower
        case 31920: // flamethrower
        case 31921: // flamethrower
			switch (ammoId) {
			case -1:
				return 3;
			case 877: // bronze bolts
