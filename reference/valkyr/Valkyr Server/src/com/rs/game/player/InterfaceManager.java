...
import com.rs.game.player.content.TaskSystem;
...
public class InterfaceManager {
	...
	public void sendTaskSystem() {
        ...
        player.getPackets().sendIComponentText(1019, 3,  "PvP Tab");
        player.getPackets().sendIComponentText(1019, 16,  "Report Player");
		if (!player.isCanPvp()) {
			player.getPackets().sendIComponentText(1019, 18, "<col=ff0000>Toggle PvP");
		} else if (player.isCanPvp()) {
			player.getPackets().sendIComponentText(1019, 18, "<col=00ff00>Toggle PvP");
		}
        player.getPackets().sendIComponentText(1019, 11,  " "); //middle
        player.getPackets().sendIComponentText(1019, 0,  "Report a player here.");//bottom understatement
        player.getPackets().sendIComponentText(1019, 8,  "Toggle your open world PvP here.");//top understatement*/
		//sendTab(resizableScreen ? 112 : 172, 917);  
		
		sendTab(resizableScreen ? 112 : 172, 930);		
		player.getPackets().sendIComponentText(930, 10, "Valkyr Information");
		player.getPackets().sendIComponentText(930, 16, "");
    }
	
	
	public void sendDiangoContainer() {
		sendInterface(554);
		player.getPackets().sendIComponentModel(554, 40, 96996);
	}
	...
	public void sendDeathShop() { //29980
		player.getInterfaceManager().sendInterface(378);
			...
			player.getPackets().sendItemOnIComponent(378, 93, 10551, 1);
			player.getPackets().sendItemOnIComponent(378, 94, 10551, 1);
			player.getPackets().sendItemOnIComponent(378, 95, 10551, 1);
			player.getPackets().sendItemOnIComponent(378, 96, 10551, 1);
			player.getPackets().sendItemOnIComponent(378, 102, 29980, 1); // tokens
			player.getPackets().sendItemOnIComponent(378, 103, 25031, 1);
			player.getPackets().sendItemOnIComponent(378, 104, 25031, 1);
			player.getPackets().sendItemOnIComponent(378, 105, 25028, 1);
			player.getPackets().sendItemOnIComponent(378, 101, 25034, 1); // max cape
			player.getPackets().sendIComponentText(378, 89,  "Current Death Task  ");
		        ...
		        player.getPackets().sendIComponentText(378, 83,  "Saradomin's hiss.");
		        player.getPackets().sendIComponentText(378, 90,  "150 Points.");
		        player.getPackets().sendIComponentText(378, 87,  "Saradomin's whisper.");
		        player.getPackets().sendIComponentText(378, 99,  "150 Points.");
		        player.getPackets().sendIComponentText(378, 88,  "Saradomin's Murmur.");
		        player.getPackets().sendIComponentText(378, 100,  "150 Points.");
		        player.getPackets().sendIComponentText(378, 84,  "Slayer XP.");
		        player.getPackets().sendIComponentText(378, 91,  "50 Points.");
		        player.getPackets().sendIComponentText(378, 85,  "Fighter Torso`.");
		        player.getPackets().sendIComponentText(378, 97,  "100 Points.");
	...
	public void sendQuestTab() {
		...
		player.getPackets().sendUnlockIComponentOptionSlots(190, 15, 0, 201, 0, 1, 2, 3); //unlocks quests
		//player.getPackets().sendConfig(160, 0); Quest state is handled by configs - fuck
		
		player.getPackets().sendConfig(904, Settings.TOTAL_QUEST_POINTS); // Quest points in the botom
		player.getPackets().sendConfig(101, player.questPoints); // Quest Points
	}
	...
	public void sendDungShop() {
		...
		player.getPackets().sendIComponentText(825, 96,  "200K");
	...
	public void sendFullScreenInterfaces() {
        ...
        WorldTasksManager.schedule(new WorldTask() {
		    ...
		    public void run() {
			    ticks++;
                if (ticks == 1) {
            		player.getPackets().sendIComponentText(746, 17, "Current Task:");
            		if (player.getSlayerManager().getCurrentTask() != null) {
            			player.getPackets().sendIComponentText(746, 183, player.getSlayerManager().getCount() + " " + player.getSlayerManager().getCurrentTask().getName());
            		} 
            		else if (player.getDeathsManager().getTask() != null) {
            			player.getPackets().sendIComponentText(746, 183, player.getDeathsManager().getAmount() + " " + NPCDefinitions.getNPCDefinitions(player.getDeathsManager().getTask().getId()).getName());
            		}
            		else {
            			player.getPackets().sendIComponentText(746, 183, "None");
            		}
            		ticks = 0;
                }
            }  
	...
	public void sendFixedInterfaces() {
        ...
        WorldTasksManager.schedule(new WorldTask() {
		    ...
		    public void run() {
			    ticks++;
                if (ticks == 1) {
            		player.getPackets().sendIComponentText(746, 17, "Owner: Andy");
            		player.getPackets().sendIComponentText(746, 183, "Owner: Corey");
            		ticks = 0;
                }
            }  
	    }, 0, 0);
	...
	player.getPackets().sendIComponentText(220, 1, "<col=24246B>The Official Noszscape Barcrawl!"); //Title
	...
	public void sendHelpBook() {
		...
		player.getInterfaceManager().sendInterface(275);
		player.getPackets().sendIComponentText(275, 1, "<col=0099FF>Vuse-RSPS Help");
		player.getPackets().sendIComponentText(275, 10, "");
		player.getPackets().sendIComponentText(275, 11, "How can I start to make money?");
		player.getPackets().sendIComponentText(275, 12, "<col=FF6600>Crafting, Bossing, Skilling");
		player.getPackets().sendIComponentText(275, 13, "<col=FF6600>Craft Dragon stones till 99 crafting and sell for huge profit");
		player.getPackets().sendIComponentText(275, 14, "");
		player.getPackets().sendIComponentText(275, 15, "Don't know where anything is?");
		player.getPackets().sendIComponentText(275, 16, "<col=00CC00>Talk to the spirit tree at home for your teleports.");
		player.getPackets().sendIComponentText(275, 17, "<col=00CC00>Type ::Train for your training areas.");
		player.getPackets().sendIComponentText(275, 18, "<col=00CC00> Shops are all around home in the buildings.");
		...
		player.getPackets().sendIComponentText(275, 25, "<col=990099>You can ask for help in Vuse-RSPS Friendchat, Corey & Andy.");
		player.getPackets().sendIComponentText(275, 26, "");
		player.getPackets().sendIComponentText(275, 27, "Need more help? Check out our forums at " + Settings.WEBSITE_LINK);
	...
	/*public void sendPlayerCustom() {
        ...
        player.getPackets().sendIComponentText(506, 6, "Forums");
        player.getPackets().sendIComponentText(506, 8, "Donate Infomation");
		...
		} else if (player.isDonator() && !player.isExtremeDonator() &&  !player.isLegendaryDonator() && !player.isSupremeDonator()) {
			player.getPackets().sendIComponentText(506, 2, "        In-Game Ranks       Rank: <col=008000><img=8>Donator</col>");
		} else if (player.isDonator() && player.isExtremeDonator() && !player.isLegendaryDonator() && !player.isSupremeDonator()) {
			player.getPackets().sendIComponentText(506, 2, "        In-Game Ranks       Rank: <col=FF0000><img=11>Extreme Donator</col>");
		} else if (player.isDonator() && player.isExtremeDonator() && player.isLegendaryDonator() && !player.isSupremeDonator()) {
			player.getPackets().sendIComponentText(506, 2, "        In-Game Ranks       Rank: <col=0066CC><img=12>Legendary Donator</col>");
		} else if (player.isDonator() && player.isExtremeDonator() && player.isLegendaryDonator() && player.isSupremeDonator()) {
			player.getPackets().sendIComponentText(506, 2, "        In-Game Ranks       Rank: <col=ffa34c><img=13>V.I.P</col>");
        } else if (player.isIronMan()) {
 			player.getPackets().sendIComponentText(506, 2, "        In-Game Ranks       Rank: <col=798683>Ironman</col>");           
		} else {
	...
	private static final int interfaceId = 1160; 
	
	private int teleportStage;
	
	public void openTeleportInterface(Player player, int stage) {
		switch (stage) {
			case 1:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, Settings.SERVER_NAME + " Teleports");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Bosses");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "Cities");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "Skilling");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "Training");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "Slayer Monsters");
				player.getPackets().sendIComponentText(interfaceId, 52, "");
				player.getPackets().sendIComponentText(interfaceId, 36, "Monsters");
				player.getPackets().sendIComponentText(interfaceId, 54, "");
				player.getPackets().sendIComponentText(interfaceId, 39, "");
				player.getPackets().sendIComponentText(interfaceId, 56, "");
				player.getPackets().sendIComponentText(interfaceId, 40, "Close");
			break;		
			case 2:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, "Bossing Teleports");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Godwars Dungeon");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "Queen Black Dragon");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "King Black Dragon");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "Kalphite Queen");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "Corporeal Beast");
				player.getPackets().sendIComponentText(interfaceId, 52, "<col=ff0000>GROUP BOSS");
				player.getPackets().sendIComponentText(interfaceId, 36, "Vorago");
				player.getPackets().sendIComponentText(interfaceId, 54, "<col=ff0000>GROUP BOSS");
				player.getPackets().sendIComponentText(interfaceId, 39, "Kalphite King");
				player.getPackets().sendIComponentText(interfaceId, 56, "");
				player.getPackets().sendIComponentText(interfaceId, 40, "Back to Beginning");
			break;
			case 3:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, "City Teleports");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Lumbridge");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "Falador");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "Varrock");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "Taverly");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "Camelot");
				player.getPackets().sendIComponentText(interfaceId, 52, "");
				player.getPackets().sendIComponentText(interfaceId, 36, "Ardougne");
				player.getPackets().sendIComponentText(interfaceId, 54, "");
				player.getPackets().sendIComponentText(interfaceId, 39, "Canifis");
				player.getPackets().sendIComponentText(interfaceId, 56, "");
				player.getPackets().sendIComponentText(interfaceId, 40, "Next");
			break;
			case 6:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, "City Teleports");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Al Kharid");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "Yanille");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "");
				player.getPackets().sendIComponentText(interfaceId, 52, "");
			...
			case 4:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, "Slayer Teleports");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Slayer Tower");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "Ancient Cavern");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "Smoking Dungeon");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "Cave Horrors");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "Fremennik Dungeon");
				player.getPackets().sendIComponentText(interfaceId, 52, "");
				player.getPackets().sendIComponentText(interfaceId, 36, " ");
				player.getPackets().sendIComponentText(interfaceId, 54, "");
				player.getPackets().sendIComponentText(interfaceId, 39, " ");
			...
			case 5:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, "Training Teleports");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Yaks");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "Experiments");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "Rock Crabs");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "Hill Giants");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "");
				player.getPackets().sendIComponentText(interfaceId, 52, "");
				player.getPackets().sendIComponentText(interfaceId, 36, "");
			...
			case 10:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, "Skilling Teleports");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Construction");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "RuneSpan");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "Lavaflow Mining");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "Agility (Gnome)");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "Agility (Barb)");
				player.getPackets().sendIComponentText(interfaceId, 52, "");
				player.getPackets().sendIComponentText(interfaceId, 36, "Farming (Livid Farm)");
				player.getPackets().sendIComponentText(interfaceId, 54, "");
				player.getPackets().sendIComponentText(interfaceId, 39, "Dungeoneering");
				player.getPackets().sendIComponentText(interfaceId, 56, "");
				player.getPackets().sendIComponentText(interfaceId, 40, "Back to Beginning");
			break;
			case 11:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, "RuneSpan Teleports");
				player.getPackets().sendIComponentText(interfaceId, 41, "Level 1-33");
				player.getPackets().sendIComponentText(interfaceId, 26, "Floor 1");
				player.getPackets().sendIComponentText(interfaceId, 44, "Level 33-66");
				player.getPackets().sendIComponentText(interfaceId, 28, "Floor 2");
				player.getPackets().sendIComponentText(interfaceId, 46, "Level 66-99");
				player.getPackets().sendIComponentText(interfaceId, 30, "Floor 3");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "");
			...
			case 12:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, "Monsters Teleports");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Tormented Demons");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "Ganodermic Beasts");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "Glacors");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "Mithril Dragons");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "Frost Dragons");
				player.getPackets().sendIComponentText(interfaceId, 52, "");
				player.getPackets().sendIComponentText(interfaceId, 36, "Dagannoth Kings");
				player.getPackets().sendIComponentText(interfaceId, 54, "");
				player.getPackets().sendIComponentText(interfaceId, 39, "");
				player.getPackets().sendIComponentText(interfaceId, 56, "");
				player.getPackets().sendIComponentText(interfaceId, 40, "Back to Beginning");
			...
			case 50:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, "Wilderness Teleports");
				player.getPackets().sendIComponentText(interfaceId, 41, "<col=00ff00>(SAFE)</col>");
				player.getPackets().sendIComponentText(interfaceId, 26, "Edgeville");
				player.getPackets().sendIComponentText(interfaceId, 44, "<col=ff0000>(PvP - Level 15)</col>");
				player.getPackets().sendIComponentText(interfaceId, 28, "Dark Fortress");
				player.getPackets().sendIComponentText(interfaceId, 46, "<col=ff0000>(PvP - Level 21)</col>");
				player.getPackets().sendIComponentText(interfaceId, 30, "Graveyard of Shadows");
				player.getPackets().sendIComponentText(interfaceId, 48, "<col=ff0000>(PvP - Level 49)</col>");
				player.getPackets().sendIComponentText(interfaceId, 32, "Wilderness Agility");
				player.getPackets().sendIComponentText(interfaceId, 50, "<col=ff0000>(PvP - Level 51)</col>");
				player.getPackets().sendIComponentText(interfaceId, 34, "Deserted Keep");
				player.getPackets().sendIComponentText(interfaceId, 52, "<col=ff0000>(PvP - Level 46)</col>");
				player.getPackets().sendIComponentText(interfaceId, 36, "Demonic Ruins");
				player.getPackets().sendIComponentText(interfaceId, 54, "");
				player.getPackets().sendIComponentText(interfaceId, 39, "");
				player.getPackets().sendIComponentText(interfaceId, 56, "");
				player.getPackets().sendIComponentText(interfaceId, 40, "Close");
			...
			case 100:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, "Minigame Teleports");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Castle Wars");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "Pest Control");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "Fight Pits");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "Warriors Guild");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "Barrows");
				player.getPackets().sendIComponentText(interfaceId, 52, "");
				player.getPackets().sendIComponentText(interfaceId, 36, "");
				player.getPackets().sendIComponentText(interfaceId, 54, "");
				player.getPackets().sendIComponentText(interfaceId, 39, "");
			...
			case 110:
				teleportStage = stage;
				player.getInterfaceManager().sendInterface(interfaceId);
				player.getPackets().sendIComponentText(interfaceId, 75, "Preset Manager");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Save Presets");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "Load Presets");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "");
			...
			case 111:
				...
				player.getPackets().sendIComponentText(interfaceId, 75, "Preset Manager");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Save Preset 1");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "Save Preset 2");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "Save Preset 3");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "Save Preset 4");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "Save Preset 5");
				player.getPackets().sendIComponentText(interfaceId, 52, "");
				player.getPackets().sendIComponentText(interfaceId, 36, "");
				player.getPackets().sendIComponentText(interfaceId, 54, "");
				player.getPackets().sendIComponentText(interfaceId, 39, "");
				player.getPackets().sendIComponentText(interfaceId, 56, "");
				player.getPackets().sendIComponentText(interfaceId, 40, "Back");
			break;
			...
			case 112:
				...
				player.getPackets().sendIComponentText(interfaceId, 75, "Preset Manager");
				player.getPackets().sendIComponentText(interfaceId, 41, "");
				player.getPackets().sendIComponentText(interfaceId, 26, "Load Preset 1");
				player.getPackets().sendIComponentText(interfaceId, 44, "");
				player.getPackets().sendIComponentText(interfaceId, 28, "Load Preset 2");
				player.getPackets().sendIComponentText(interfaceId, 46, "");
				player.getPackets().sendIComponentText(interfaceId, 30, "Load Preset 3");
				player.getPackets().sendIComponentText(interfaceId, 48, "");
				player.getPackets().sendIComponentText(interfaceId, 32, "Load Preset 4");
				player.getPackets().sendIComponentText(interfaceId, 50, "");
				player.getPackets().sendIComponentText(interfaceId, 34, "Load Preset 5");
				player.getPackets().sendIComponentText(interfaceId, 52, "");
				player.getPackets().sendIComponentText(interfaceId, 36, "");
				player.getPackets().sendIComponentText(interfaceId, 54, "");
				player.getPackets().sendIComponentText(interfaceId, 39, "");
			...
			default:
				player.sm("Invalid stage ID: " + stage);
		}
	}
	...
	public void handleButtons(Player player, int buttonId) {
		switch (buttonId) {
			...
			case 42: //button 1
				if (teleportStage == 1) {
					openTeleportInterface(player, 2);
				} else if (teleportStage == 2) {
					TeleportManager.teleportGodwars(player, new WorldTile(2872, 5314, 0));
				} else if (teleportStage == 3) {
					TeleportManager.teleportNormal(player, new WorldTile(3222, 3218, 0), true);
				} else if (teleportStage == 4) {
					TeleportManager.teleportNormal(player, new WorldTile(2223, 3340, 0), true);
				} else if (teleportStage == 5) {
					TeleportManager.teleportNormal(player, new WorldTile(2324, 3793, 0), true);
				} else if (teleportStage == 6) {
					TeleportManager.teleportNormal(player, new WorldTile(3293, 3183, 0), true);
				} else if (teleportStage == 10) {
					if (player.getHouse() != null) {
						player.getHouse().joinHouse(player);
						closeScreenInterface();
					} else {
						player.sm("You can purchase a house from any real estate agent.");
					}
				} else if (teleportStage == 11) {
					TeleportManager.teleportNormal(player, new WorldTile(3993, 6108, 1), true);
					player.getControlerManager().startControler("RunespanControler");
				} else if (teleportStage == 12) {
					TeleportManager.teleportNormal(player, new WorldTile(2563, 5740, 0), true);
				} else if (teleportStage == 50) {
					TeleportManager.teleportNormal(player, new WorldTile(3087, 3501, 0), false);
				} else if (teleportStage == 100) {
					TeleportManager.teleportNormal(player, new WorldTile(2442, 3090, 0), true);
				} else if (teleportStage == 110) {
					openTeleportInterface(player, 111);
				} else if (teleportStage == 111) {
					player.getPresetManager().savePreset(1);
					closeScreenInterface();
				} else if (teleportStage == 112) {
					player.getPresetManager().loadPreset(1);
					closeScreenInterface();
				} else {
					player.sm("Nothing interesting happends.");
				}
			break;
			
			case 45: //button 2
				if (teleportStage == 1) {
					openTeleportInterface(player, 3);
				} else if (teleportStage == 2) {
					if (player.getSkills().getLevelForXp(Skills.SUMMONING) < 60) {
						player.getPackets().sendGameMessage("You need a Summoning level of 60 to start this boss.");
						return;
					}
					player.getControlerManager().startControler("QueenBlackDragonControler");
					closeScreenInterface();
				} else if (teleportStage == 3) {
					TeleportManager.teleportNormal(player, new WorldTile(2965, 3379, 0), true);
				} else if (teleportStage == 4) {
					TeleportManager.teleportNormal(player, new WorldTile(1738, 5312, 1), true);
				} else if (teleportStage == 5) {
					TeleportManager.teleportNormal(player, new WorldTile(3555, 9947, 0), true);
				} else if (teleportStage == 6) {
					TeleportManager.teleportNormal(player, new WorldTile(2603, 3087, 0), true);
				} else if (teleportStage == 10) {
					openTeleportInterface(player, 11);
				} else if (teleportStage == 11) {
					if (player.getSkills().getLevel(Skills.RUNECRAFTING) < 33) {
						player.sm("You need to have at least level 33 Runecrafting to enter here.");
						return;
					}
					TeleportManager.teleportNormal(player, new WorldTile(4137, 6089, 1), true);
					player.getControlerManager().startControler("RunespanControler");
				} else if (teleportStage == 12) {
					TeleportManager.teleportNormal(player, new WorldTile(4630, 5404, 0), true);
				} else if (teleportStage == 50) {
					TeleportManager.teleportWilderness(player, new WorldTile(3006, 3634, 0), false);
				} else if (teleportStage == 100) {
					TeleportManager.teleportNormal(player, new WorldTile(2658, 2661, 0), true);
				} else if (teleportStage == 110) {
					openTeleportInterface(player, 112);
				} else if (teleportStage == 111) {
					player.getPresetManager().savePreset(2);
					closeScreenInterface();
				} else if (teleportStage == 112) {
					player.getPresetManager().loadPreset(2);
					closeScreenInterface();
				} else {
					player.sm("Nothing interesting happends.");
				}
			...
			case 47: //button 3
				if (teleportStage == 1) {
					openTeleportInterface(player, 10);
				} else if (teleportStage == 2) {
					TeleportManager.teleportNormal(player, new WorldTile(2273, 4681, 0), true);
				} else if (teleportStage == 3) {
					TeleportManager.teleportNormal(player, new WorldTile(3212, 3423, 0), true);
				} else if (teleportStage == 4) {
					player.getDialogueManager().startDialogue("SmokeDungeonEntrance");
				} else if (teleportStage == 5) {
					TeleportManager.teleportNormal(player, new WorldTile(2706, 3714, 0), true);
				} else if (teleportStage == 10) {
					TeleportManager.teleportNormal(player, new WorldTile(2182, 5664, 0), true);
				} else if (teleportStage == 11) {
					if (player.getSkills().getLevel(Skills.RUNECRAFTING) < 66) {
						player.sm("You need to have at least level 66 Runecrafting to enter here.");
						return;
					}
					TeleportManager.teleportNormal(player, new WorldTile(4295, 6038, 1), true);
					player.getControlerManager().startControler("RunespanControler");
				} else if (teleportStage == 12) {
					TeleportManager.teleportNormal(player, new WorldTile(4196, 5750, 0), true);
				} else if (teleportStage == 50) {
					TeleportManager.teleportWilderness(player, new WorldTile(3226, 3683, 0), false);
				} else if (teleportStage == 100) {
					TeleportManager.teleportNormal(player, new WorldTile(4601, 5061, 0), true);
				} else if (teleportStage == 111) {
					player.getPresetManager().savePreset(3);
					closeScreenInterface();
				} else if (teleportStage == 112) {
					player.getPresetManager().loadPreset(3);
					closeScreenInterface();
				} else {
					player.sm("Nothing interesting happends.");
				}
			...
			case 49: //button 4
				if (teleportStage == 1) {
					openTeleportInterface(player, 5);
				} else if (teleportStage == 2) {
					TeleportManager.teleportNormal(player, new WorldTile(3505, 9494, 0), true);
				} else if (teleportStage == 3) {
					TeleportManager.teleportNormal(player, new WorldTile(2881, 3442, 0), true);
				} else if (teleportStage == 4) {
					TeleportManager.teleportNormal(player, new WorldTile(3741, 9373, 0), true);
				} else if (teleportStage == 5) {
					TeleportManager.teleportNormal(player, new WorldTile(3116, 9843, 0), true);
				} else if (teleportStage == 10) {
					TeleportManager.teleportNormal(player, new WorldTile(2474, 3438, 0), true);
				} else if (teleportStage == 12) {
					TeleportManager.teleportNormal(player, new WorldTile(1778, 5342, 1), true);
				} else if (teleportStage == 50) {
					TeleportManager.teleportWilderness(player, new WorldTile(2998, 3911, 0), false);
				} else if (teleportStage == 100) {
					TeleportManager.teleportNormal(player, new WorldTile(2880, 3541, 0), true);
				} else if (teleportStage == 111) {
					player.getPresetManager().savePreset(4);
					closeScreenInterface();
				} else if (teleportStage == 112) {
					player.getPresetManager().loadPreset(4);
					closeScreenInterface();
				} else {
					player.sm("Nothing interesting happends.");
				}
			...
			case 51:// button 5
				if (teleportStage == 1) {
					openTeleportInterface(player, 4);
				} else if (teleportStage == 2) {
					TeleportManager.teleportNormal(player, new WorldTile(2966, 4383, 2), true);
				} else if (teleportStage == 3) {
					TeleportManager.teleportNormal(player, new WorldTile(2757, 3478, 0), true);
				} else if (teleportStage == 4) {
					TeleportManager.teleportNormal(player, new WorldTile(2792, 3615, 0), true);
				} else if (teleportStage == 10) {
					TeleportManager.teleportNormal(player, new WorldTile(2552, 3563, 0), true);
				} else if (teleportStage == 12) {
					TeleportManager.teleportNormal(player, new WorldTile(3033, 9596, 0), true);
				} else if (teleportStage == 50) {
					TeleportManager.teleportWilderness(player, new WorldTile(3155, 3923, 0), false);
				} else if (teleportStage == 100) {
					TeleportManager.teleportNormal(player, new WorldTile(3566, 3317, 0), true);
				} else if (teleportStage == 111) {
					player.getPresetManager().savePreset(5);
					closeScreenInterface();
				} else if (teleportStage == 112) {
					player.getPresetManager().loadPreset(5);
					closeScreenInterface();
				} else { 
					player.sm("Nothing interesting happends.");
				}
			...
			case 53: //button 6
				if (teleportStage == 1) {
					openTeleportInterface(player, 12);
				} else if (teleportStage == 2) {
					if (player.getRights() >= 2) {
						TeleportManager.teleportNormal(player, new WorldTile(3545, 9507, 0), true);
						return;
					}
					player.sm("Currently removed until polished up.");
					closeScreenInterface();
				} else if (teleportStage == 3) {
					TeleportManager.teleportNormal(player, new WorldTile(2659, 3306, 0), true);
				} else if (teleportStage == 10) {
					TeleportManager.teleportNormal(player, new WorldTile(2111, 3946, 0), true);
					player.sm("Talk to lady Servil to get started.");
				} else if (teleportStage == 12) {
					TeleportManager.teleportNormal(player, new WorldTile(1902, 4366, 0), true);
				} else if (teleportStage == 50) {
					TeleportManager.teleportWilderness(player, new WorldTile(3288, 3886, 0), false);
				} else { 
					player.sm("Nothing interesting happends.");
				}
			break;
			...
			case 55: //button 7
				if (teleportStage == 1) {
					openTeleportInterface(player, 1);
				} else if (teleportStage == 2) {
					TeleportManager.teleportNormal(player, new WorldTile(3219, 2803, 0), true);
				} else if (teleportStage == 3) {
					TeleportManager.teleportNormal(player, new WorldTile(3495, 3491, 0), true);
				} else if (teleportStage == 10) {
					TeleportManager.teleportNormal(player, new WorldTile(3450, 3727, 0), true);
				} else { 
					player.sm("Nothing interesting happends.");
				}
			break;
			...
			case 57: //button 8
				if (teleportStage == 1 || teleportStage == 50 || teleportStage == 100 
						|| teleportStage == 110)
					closeScreenInterface();
				else if (teleportStage == 3) 
					openTeleportInterface(player, 6);
				else if (teleportStage == 111 || teleportStage == 112)
					openTeleportInterface(player, 110);
				else 
					openTeleportInterface(player, 1);
			break;
			
			default:
				player.sm("This button has not been programmed yet.");
		}
	}
    
    //End Teleport interface
	
    public void sendPerk(String perk) {
		player.setPerk(perk, true);
		player.getInterfaceManager().sendOverlay(1073, false);
		player.getPackets().sendIComponentText(1073, 10, "<col=00ff00>Perk unlocked!");
		player.getPackets().sendIComponentText(1073, 11, "<col=00ff00>" + perk.substring(0, 1).toUpperCase() + perk.substring(1));
		player.getPackets().sendGameMessage("You have unlocked the perk: <col=800000>" + perk.substring(0, 1).toUpperCase() + perk.substring(1));
		new Timer().schedule(new TimerTask() {
			@Override
		    public void run() {
				player.getInterfaceManager().closeOverlay(false);
		    }
		}, 6000); //thread sleep timer
	}
