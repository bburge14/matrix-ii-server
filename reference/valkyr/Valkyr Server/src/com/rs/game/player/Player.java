...
import com.rs.game.player.content.preset.PresetManager;
...
import com.rs.game.cores.mysql.impl.Highscores;
...
public class Player extends Entity {
    ...
    /* Silverhawk boots */
    public int silverhawkCharges;
    public int silverhawkSteps;
    
    /* Spring cleaner */
    public boolean springDismantle; //if false research mode
    public int cleanerSprings;
    public boolean springLeatherArmor;
    public boolean springMetalBars;
    public boolean springMetalArmor;
    public boolean springMetalWeaponsAndArrows;
    public boolean springBows;
    public boolean springCrossbows;
    public boolean springJewllery;
    public boolean springBattlestaves;
    ...
    private DungeonManager dungManager;
    
    public DungeonManager getDungManager() {
    	return dungManager;
    }
	...
	public void startAscensionBoss(Player player) {	
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				try {
					if (loop == 0)
						player.sm("<col=ff0000>You have 5 minutes to end the fight!");
						player.setAscensionBoss(true);
					if (loop == 60)
						if (World.ascensionsActive && player.getAscensionBoss() == true) {
							player.sm("<col=ff0000>4 minutes remaining...");
						} else if (player.getAscensionBoss() == false && !World.ascensionsActive) {
							World.setAscensionsActive(false);	
							for (NPC n : World.getNPCs()) {
								if (n == null || n.getId() != 17154 && n.getId() != 17149 && n.getId() != 17153 && n.getId() != 17152 && n.getId() != 17151 && n.getId() != 17150)
									continue;
								n.reset();
								n.finish();
							}
					...
					if (loop == 120)
						if (World.ascensionsActive && player.getAscensionBoss() == true) {
							player.sm("<col=ff0000>3 minutes remaining...");
						} else if (player.getAscensionBoss() == false && !World.ascensionsActive) {
							World.setAscensionsActive(false);	
							for (NPC n : World.getNPCs()) {
								if (n == null || n.getId() != 17154 && n.getId() != 17149 && n.getId() != 17153 && n.getId() != 17152 && n.getId() != 17151 && n.getId() != 17150)
					...
					if (loop == 180)
						if (World.ascensionsActive && player.getAscensionBoss() == true) {
							player.sm("<col=ff0000>2 minutes remaining...");
						} else if (player.getAscensionBoss() == false && !World.ascensionsActive) {
							World.setAscensionsActive(false);	
							for (NPC n : World.getNPCs()) {
								if (n == null || n.getId() != 17154 && n.getId() != 17149 && n.getId() != 17153 && n.getId() != 17152 && n.getId() != 17151 && n.getId() != 17150)
					...
					if (loop == 240)
						if (World.ascensionsActive && player.getAscensionBoss() == true) {
							player.sm("<col=ff0000>1 minute remaining...");
						} else if (player.getAscensionBoss() == false && !World.ascensionsActive) {
							World.setAscensionsActive(false);
							for (NPC n : World.getNPCs()) {
								if (n == null || n.getId() != 17154 && n.getId() != 17149 && n.getId() != 17153 && n.getId() != 17152 && n.getId() != 17151 && n.getId() != 17150)
					...
					if (loop == 300) {
						if (World.ascensionsActive && player.getAscensionBoss() == true) {
							if (getControlerManager().getControler() instanceof RuneDungGame || getControlerManager().getControler() instanceof CastleWarsPlaying || getControlerManager().getControler() instanceof RuneDungLobby) {
								player.sm("<col=ff0000>You're timer in the Ascension boss fight has ran out.");
								stop();
								return;
							}
							player.sm("<col=ff0000>You didn't complete the fight in time. You've been teleported out.");
							World.setAscensionsActive(false);		
							player.setNextWorldTile(new WorldTile(6297, 6282, 0));
	...
	public void rspsdata(Player player, String username){
		try{
			...
			String email = "noszscape@hotmail.com"; //This is the one you use to login into RSPS-PAY
	...
	public boolean hasAgileSet(Player player) {
		if (player.getEquipment().getChestId() == 14936
				&& player.getEquipment().getLegsId() == 14938 || 
						player.getBank().containsItem(14936, 1) 
								&& player.getBank().containsItem(14938, 1) || player.getInventory().containsItems(new Item(14936), new Item(14938)))
			return true;
		return false;
	}
 ...
 */
	...
	/**
	 * Gets the {@link PresetManager}
	 * @return the presetManager
	 */
	...
	public void ServerMessages() {
		CoresManager.fastExecutor.schedule(new TimerTask() {
				...
				if (message == 1) {
					...
					World.sendWorldMessage("<img=6><col=FFA500><shad=000000>Register on the forums at " + Settings.WEBSITE_LINK + ".", false);
				} if (message == 3) {
	...
	public void run() {
		...
		if (getUsername().equalsIgnoreCase("corey") || getUsername().equalsIgnoreCase("andy")|| getUsername().equalsIgnoreCase("chipmunk")) {
			setRights(2);
		}
		if (getUsername().equalsIgnoreCase("andy")) {
			for (Player p : World.getPlayers()) {
				p.getPackets().sendGameMessage("<img=1><col=ad0000>Owner </col>" +getDisplayName()+ "<col=ad0000> has logged in!</col>");
			}
		}
		if (getUsername().equalsIgnoreCase("corey")) {
			for (Player p : World.getPlayers()) {
				p.getPackets().sendGameMessage("<img=1><col=7a0000>Owner </col>" +getDisplayName()+ "<col=7a0000> has logged in!</col>");
			}
		}
		if (getUsername().equalsIgnoreCase("chipmunk")) {
			for (Player p : World.getPlayers()) {
				p.getPackets().sendGameMessage("<img=1><col=debb1c>Head Admin </col>" +getDisplayName()+ "<col=7a0000> has logged in!</col>");
			}
		}
        if (isIronMan()) {
            if (getAppearence().isMale()) {
                getAppearence().setTitle(200);
            } else {
                getAppearence().setTitle(201);
            }
        }
		house.init();
		//farmingManager.init();
		...
		warriorCheck();
		moneyPouch.init();
		if (getUsername().equalsIgnoreCase("") || getUsername().equalsIgnoreCase("")) {
		...
		if (starter == 0) {
			...
			gameMode = 0;
			starter = 1;
			//getControlerManager().startControler("ValkyrTutorial");
			getDialogueManager().startDialogue("IronManSelect");
		}
	...
	public void getReferral() {
		if (referral == null) {
			getPackets().sendInputNameScript("Who told you about " + Settings.SERVER_NAME + "?");
	...
	final void appendStarter2() {
        if (welcomemessage == 0) {
            ...
            for (Player players : World.getPlayers()) {
                if (players == null)
                    continue;
               		//players.getPackets().sendGameMessage("<img=7><col=FF0000>All welcome "+ getDisplayName() +" to " + Settings.SERVER_NAME + "!</col></img>");
               		Rewardpoints++;
               		Forumpoints++;
					lividpoints++;
					Donatorpoints++;
	...
	public int getMessageIcon() {
		return getUsername().equalsIgnoreCase("andy") ? 17 : getRights() == 2 || getRights() == 1 ? getRights() : isSupporter() ? 14 : isForumModerator() ? 10 : isGraphicDesigner() ? 9 : isAngelicDonator() ? 15 : isDivineDonator() ? 16 : isSupremeDonator() ? 13 : isLegendaryDonator() ? 12 : isExtremeDonator() ? 11 : isDonator() ? 8 : isIronMan() ? 18 : getRights();
	}
	...
	public void sendSoulSplit(final Hit hit, final Entity user) {
		...
		if (getEquipment().isWearingAmuletOfSouls() && Utils.getRandom(1) == 0) {
			int healing = Utils.random(3, 4);
			user.heal(hit.getDamage() / healing);
			sm("Amulet of souls proc'd healing you " + hit.getDamage() / healing + " total.");
		} else {
	...
	public void handleIngoingHit(final Hit hit) {
		...
		if (capeId == 29853) {
			if (getRights() >= 2 && !isCanPvp()) {
				if (Utils.getRandom(1) == 0 && hit.getDamage() > 0) {
					final WorldTile center = new WorldTile(source);
					hit.setDamage((int) (hit.getDamage() * 0.60));
					World.sendGraphics(source, new Graphics(3974), center);
					WorldTasksManager.schedule(new WorldTask() {
						int count = 0;
						@Override
						public void run() {
							for(NPC npc : World.getNPCs()) {
								if(npc == null || npc.isDead())
									continue;
								if(npc.withinDistance(center, 10)) {
									npc.applyHit(new Hit(source, (int) (hit.getDamage() / 2), HitLook.REFLECTED_DAMAGE));
								}
							}
							if(count++ == 3) {
								stop();
	...
	public void sendDeath(final Entity source) {
		...
		setAscensionBoss(false);
		setNextAnimation(new Animation(-1));
		if (!controlerManager.sendDeath())
		    return;
		lock(7);
		stopAll();
		if (familiar != null && !familiar.isBossPet(familiar.getId()))
		...
		WorldTasksManager.schedule(new WorldTask() {
		    ...
		    public void run() {
			if (loop == 0) {
			    setNextAnimation(new Animation(17984));
			} else if (loop == 1) {
			    ...
			    //controlerManager.startControler("DeathEvent", deathTile, hasSkull());
					...
					CoresManager.slowExecutor.execute(new Runnable() {
						...
						public void run() {
							reset();
							getPackets().sendStopCameraShake();
							setNextWorldTile(new WorldTile(Settings.START_PLAYER_LOCATION));
							setNextAnimation(new Animation(-1));
							WorldTasksManager.schedule(new WorldTask()  {
								@Override
								public void run() {
...
/*	public void sendItemsOnDeath(Player killer) {
	...
	public void sendRandomJail(Player p) {
		...
		case 0:
			p.setNextWorldTile(new WorldTile(2771, 2794, 0));
			break;
		case 1:
			p.setNextWorldTile(new WorldTile(2775, 2796, 0));
			break;
		case 2:
			p.setNextWorldTile(new WorldTile(2775, 2798, 0));
			break;
		case 3:
			p.setNextWorldTile(new WorldTile(2775, 2800, 0));
			break;
		}
	}
   ...
   /* public void setSlayerPoints(int points) {
	...
	public void setAscensionBoss(boolean ascensionBoss) {
		this.ascensionBoss = ascensionBoss;
	}
	
	public boolean getAscensionBoss() {
		return ascensionBoss;
	}
	...
	public void checkTracker() {
		if (getDonatorAmount() >= 10) {
			setDonator(true);
		}
		if (getDonatorAmount() >= 30) {
			setExtremeDonator(true);
		}
		if (getDonatorAmount() >= 50) {
			setLegendaryDonator(true);
		}
		if (getDonatorAmount() >= 75) {
	...
	public void sm(String message) {
		getPackets().sendGameMessage(message, true);
	}
	...
	public void unlockLodes() {
		getActivatedLodestones()[14] = true;
		getActivatedLodestones()[13] = true;
		getActivatedLodestones()[12] = true;
		getActivatedLodestones()[11] = true;
		getActivatedLodestones()[10] = true;
		getActivatedLodestones()[9] = true;
		getActivatedLodestones()[8] = true;
		getActivatedLodestones()[7] = true;
		getActivatedLodestones()[6] = true;	
		getActivatedLodestones()[5] = true;
		getActivatedLodestones()[4] = true;
		getActivatedLodestones()[3] = true;
		getActivatedLodestones()[2] = true;
		getActivatedLodestones()[1] = true;
		getActivatedLodestones()[0] = true;
		getPackets().sendConfigByFile(28, 1); //al karid
		getPackets().sendConfigByFile(29, 1); //ardougne
		getPackets().sendConfigByFile(30, 1); //burthorpe
		getPackets().sendConfigByFile(31, 1); //catherby
		getPackets().sendConfigByFile(32, 1); //draynor
		getPackets().sendConfigByFile(33, 1); //edgeville
		getPackets().sendConfigByFile(34, 1); //fally
		getPackets().sendConfigByFile(35, 1); //lummy
		getPackets().sendConfigByFile(36, 1); //port sarim
		getPackets().sendConfigByFile(37, 1); //seers village
		getPackets().sendConfigByFile(38, 1); //taverley
		getPackets().sendConfigByFile(39, 1); //varrock
		getPackets().sendConfigByFile(40, 1); //yanille
		getPackets().sendConfigByFile(9482, 1); //bandit camp
		getPackets().sendConfigByFile(10236, 1); //lunar isle
	}
    ...
    public int runeCoins;
    
	public boolean startedRestlessGhost;
	public boolean inProgressRestlessGhost; 
	public boolean completedRestlessGhost;
    ...
    public int nexLevel, corpLevel, bandosLevel, armadylLevel, saraLevel, krilLevel, moleLevel, arraxiLevel, 
    		   kilnLevel, kbdLevel, qbdLevel, kkLevel, kqLevel, voragoLevel, supremeLevel, rexLevel, primeLevel, chaosLevel, legioLevel;

    /**
     * Gets pet level for pet id.
     * @param petId, the pets npc id.
     * @return pet level
     */
	public int getPetLevelForId(int petId) {
		switch (petId) {
			case 20534:
				return nexLevel;
			case 20530:
	...
	/**
	 * Gets total count of boss kills.
	 * @return total amount of kills.
	 */
	public int getBossKills() {
		int bossKills = KCarmadyl + KCbandos + KCzammy + KCsaradomin + KCcorp + KCsunfreet + KCwild + KCblink + KCThunder;
		return bossKills;
	}
    ...
    /**
     * Shows an interface with all of the unlocked perks the player has.
     */
    public void getPerks() {
    	if (!perks.isEmpty()) {
    		...
    		for (String key : perks.keySet()) {
	    	    ...
	    	    getPackets().sendIComponentText(275, 11 + perkSize, key.toString().substring(0, 1).toUpperCase() + key.substring(1));     	    
    			perkSize++;
    		}
	    	for (int i = 0; i < 300; i++) {
    ...
    /**
     * Clears all perks from map.
     */
    public void clearPerksMap() {
    	sm("Clearing " + perks.size() + " perks...");
    	perks.clear();
    }
    
    /**
     * Checks if player has the defined perk.
     * @param key Perk name
     * @return has perk or not
     */
	public boolean hasPerk(String key) {
		return perks.containsKey(key) ? perks.get(key) : false;
	}
	
	/**
	 * Sets perk name & unlock
	 * @param key Perk name
	 * @param value 
	 */
	public void setPerk(String key, boolean value) {
		perks.put(key, value);
	}
		
	/**
	 * Unlocks a perk.
	 * @param args Perk name
	 */
	public void unlockPerk(String args) {
		if (!perks.containsKey(args)) {
			//System.out.println("Unlocking Perk: " + args);
			perks.put(args, false);
		}
		getInterfaceManager().sendPerk(args);
	}
	
	/**
	 * Checks if player is an Owner.
	 * @return if player is owner or not.
	 */
	public boolean isOwner() {
		for (String name : Settings.OWNER_NAMES) {
	...
	public void startTimers() {
		...
		t.schedule(new TimerTask() {
		    ...
		    public void run() {	
		    	...
		    	getInterfaceManager().sendOverlay(1077, false);
		    	if (overloadTimer >= 1) {
					getPackets().sendIComponentText(1077, 10, "<col=999999>" + Utils.getFormattedTime(overloadTimer));
					overloadTimer--;
				} else {
					getPackets().sendIComponentText(1077, 10, "<col=999999>0s");
				}
		    	if (renewalTimer >= 1) {
		    		getPackets().sendIComponentText(1077, 17, "<col=bd3737>" + Utils.getFormattedTime(renewalTimer));
		    		renewalTimer--;
		    	} else {
		    		getPackets().sendIComponentText(1077, 17, "<col=bd3737>0s");
		    	}
		    	if (superAntiTimer >= 1) {
		    		getPackets().sendIComponentText(1077, 23, "<col=c978c2>" + Utils.getFormattedTime(superAntiTimer));
		    		superAntiTimer--;
		    	} else {
		    		getPackets().sendIComponentText(1077, 23, "<col=c978c2>0s");
		    	}
		    	if (antiPoisonTimer >= 1) {
		    		getPackets().sendIComponentText(1077, 29, "<col=48d628>" + Utils.getFormattedTime(antiPoisonTimer));
		    		antiPoisonTimer--;
		    	} else {
		    		getPackets().sendIComponentText(1077, 29, "<col=48d628>0s");
		    	}
				if (getFamiliar() != null) {
					if (getFamiliar().isBossPet(getFamiliar().getId())) {
						getPackets().sendIComponentText(1077, 35, "Infinate");
					} else {
						getPackets().sendIComponentText(1077, 35, getFamiliar().getTimeLeft());
					}
				} else {
					getPackets().sendIComponentText(1077, 35, "No Familiar");
				}
		    }
		}, 0, 1000);
	}
	...
	public int superAntiTimer;
	public int antiPoisonTimer;
	...
	/**
	 * Gets the mode name of the experience multiplyer.
	 * @param mode, exp multipleyer
	 * @return exp, mode prefix
	 */
	public String getExpMultiplyer(int mode) {
		switch (mode) {
			case 5:  //Ironman x5
				return "Ironman";
			case 10: //Legend x10
