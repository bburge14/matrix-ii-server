...
public class PrifddinasPickpocketing extends Action {
	...
	public boolean process(Player player) {
		...
		if (checkAll(player)) {
			...
			if (Utils.random(player.getSkills().getLevel(Skills.THIEVING) * 10) == 0) {
				...
				player.getPackets().sendGameMessage("You have raised their suspicions.");
	...
	public int processWithDelay(Player player) {
		...
		player.getPackets().sendGameMessage("You've successfully pickpocketed the " + npc.getName() + ".");
		addItems(player);
		return 1;
	}
	...
	private void addItems(Player player) {
		...
		player.getMoneyPouch().sendDynamicInteraction(Utils.random(120, 350), false);
