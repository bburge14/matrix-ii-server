			...
			if (p.clueLevel == 0) {
				...
				if (Misc.random(10) == 1) {
					...
					rewards = new Item[] {
					        ...
					        String reward2 = ItemDefinitions.getItemDefinitions(EliteRareRewards.length).getName().toLowerCase();
					        World.sendWorldMessage("<img=5><col=FFFF00>" + p.getDisplayName() + "<col=FFFF00> has received<col=FFFF00> " + reward2 + "<col=FFFF00> from a Elite clue!<img=5>", false);
				}
			}
