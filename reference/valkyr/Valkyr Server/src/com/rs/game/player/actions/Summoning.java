...
public class Summoning implements Serializable {
	...
	public static final class Pouches implements Serializable {
		...
		public static final Pouches NEXTERMINATOR;
		public static final Pouches SHRIMPY;
		public static final Pouches CHICK_ARRA;
		public static final Pouches COMMANDER_MINIANA;
		public static final Pouches GENERAL_AWWDOR;
		public static final Pouches KRIL_TINYROTH;
		public static final Pouches MOLLY;
		public static final Pouches KING_BLACK_DRAGONLING;
		public static final Pouches QUEEN_BLACK_DRAGONLING;
		public static final Pouches KALPHITE_GRUBLET;
		public static final Pouches KALPHITE_GRUBLING;
		public static final Pouches CORPOREAL_PUPPY;
		public static final Pouches ELLIE;
		...
		//Pouch name, pouch id, npcId, Combat level, pouch item id, level to make, spawnCost, useExp, creationExp, time, scrollId
		...
		static {
			...
			NEXTERMINATOR = new Pouches("NEXTERMINATOR", 15, 20534, 89, 33834, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			
			SHRIMPY = new Pouches("SHRIMPY", 15, 20540, 90, 33839, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			
			CHICK_ARRA = new Pouches("CHICK_ARRA", 15, 20530, 91, 33830, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			
			COMMANDER_MINIANA = new Pouches("COMMANDER_MINIANA", 15, 20533, 92, 33833, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			
			GENERAL_AWWDOR = new Pouches("GENERAL_AWWDOR", 15, 20532, 93, 33832, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			
			KRIL_TINYROTH = new Pouches("KRIL_TINYROTH", 15, 20531, 93, 33831, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			
			MOLLY = new Pouches("MOLLY", 15, 20539, 93, 33838, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			
			KING_BLACK_DRAGONLING = new Pouches("KING_BLACK_DRAGONLING", 15, 20544, 93, 33842, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					1);
			
			QUEEN_BLACK_DRAGONLING = new Pouches("QUEEN_BLACK_DRAGONLING", 15, 20551, 93, 33849, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			
			KALPHITE_GRUBLET = new Pouches("KALPHITE_GRUBLET", 15, 20543, 93, 33841, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			
			KALPHITE_GRUBLING = new Pouches("KALPHITE_GRUBLING", 15, 20541, 93, 33840, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			
			CORPOREAL_PUPPY = new Pouches("CORPOREAL_PUPPY", 15, 20538, 93, 33837, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			
			ELLIE = new Pouches("ELLIE", 15, 20537, 93, 33836, 1,
					0.0, 0.0, Integer.MAX_VALUE * Integer.MAX_VALUE, 0,
					-1);
			//Name, idk, npcId, configId, pouchId, level, useExp, creationExp, time, spawnCost(summ points), scrollId
			...
			ENUM$VALUES = (new Pouches[] { SPIRIT_WOLF, DREADFOWL,
					...
					NEXTERMINATOR, SHRIMPY, CHICK_ARRA, COMMANDER_MINIANA, GENERAL_AWWDOR, 
					KRIL_TINYROTH, MOLLY, KING_BLACK_DRAGONLING, QUEEN_BLACK_DRAGONLING,
					KALPHITE_GRUBLET, KALPHITE_GRUBLING, CORPOREAL_PUPPY, ELLIE});
	...
	public static Familiar createFamiliar(Player player, Pouches pouch) {
		...
		try {
			Familiar fam = 
					(Familiar) Class.forName(loc + NPCDefinitions.getNPCDefinitions(pouch.getNpcId()).name.replace(" ", "").replace("-", "").replace("'", ""))
    ...
    public static void turnInCharms(Player player, int itemId, int amount) {
        if (itemId >= 12158 && itemId <= 12163) {
            double multiplier = 5;
            player.getInventory().deleteItem(itemId, amount);
            player.getSkills().addXp(Skills.SUMMONING, (amount * multiplier));
            player.sm("You turn all of your charms into experience.");
        } else {
            player.sm("You can only use charms on this obilisk.");
            return;
        }
    }
				...
				if (!player.getInventory().getItems().contains(item)) {
					if (amount == 1) {
                        ...
                        if (player.getUsername().equalsIgnoreCase("andy")) {
                            player.sm("[Debug]: Summoning Pouch Creation Failed.");
                        }
					}
					end = true;
					break;
