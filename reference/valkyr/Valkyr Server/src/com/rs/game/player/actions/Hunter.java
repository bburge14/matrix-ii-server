...
public class Hunter {
	...
	public enum FlyingEntities {
		...
		YOUNG_IMPLING(1, 1, 48, 65, 22, new Item[] { new Item(361, 1), new Item(1901, 1), new Item(1539, 5), new Item(1784, 4), new Item(1523, 1), new Item(7936, 1), new Item(5970, 1) }, new Item[] { new Item(855, 1), new Item(1353, 1), new Item(2293, 1), new Item(7178, 1), new Item(247, 1), new Item(453, 1), new Item(1777, 1), new Item(231, 1), new Item(2347, 1) }, new Item[] { new Item(1097, 1), new Item(1157, 1), new Item(8778, 1), new Item(133, 1), new Item(2359, 1), }, null),
		...
		GameEngine.get().addTask(new Task() {
			...
			public void execute() {
				if (isSuccessful(player, instance.getLevel(), new DynamicFormula() {
					...
					player.sm("...and you successfully caputure the " + name.toLowerCase());
				} else {
					if (isImpling) {
						...
						WorldTasksManager.schedule(new WorldTask() {
							...
							@Override
							public void run() {
								WorldTile teleTile = npc.getWorldTile();
								for (int trycount = 0; trycount < 10; trycount++) {
									teleTile = new WorldTile(npc.getWorldTile(), 10);
									if (World.canMoveNPC(npc.getPlane(), teleTile.getX(), teleTile.getY(), player.getSize()))
										break;
									teleTile = npc.getWorldTile();
								}
								npc.setNextWorldTile(teleTile);
							}
						}, 0, 1);
					}
					player.sm("...you stumble and miss the " + name.toLowerCase());
				}
			}
		});
	}
	...
	public static void openJar(Player player, FlyingEntities instance) {
		...
		if (Utils.random(100) > 93) {
			...
			player.sm("You press too hard on the jar and the glass shatters in your hands.");
		}
	}
	...
	public static void createLoggedObject(Player player, WorldObject object, boolean kebbits) {
		...
		if (World.removeTemporaryObject(object, 300000, false)) {
			World.spawnTemporaryObject(new WorldObject(kebbits ? 19206 : -1, object.getType(), object.getRotation(), object.getLocation(object)), 300000, false);// TODO
			Item item = null;
