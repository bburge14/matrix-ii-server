...
import com.rs.game.npc.others.Airut;
...
public final class World {
	...
	public static boolean ascensionsActive;
	...
	public static void setAscensionsActive(boolean ascensionsActive) {
		World.ascensionsActive = ascensionsActive;
	}
	
	public static boolean isAscensions() {
		return ascensionsActive;
	}
	...
	public static final NPC spawnNPC(int id, WorldTile tile,
		...
		if (hunterNPCs != null) {
			...
			n = new KingBlackDragon(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, 
					spawned);
		...
		else if (id == 2894 || id == 2892)
			n = new Spinolyp(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
		else if (id == 17154 || id == 17153 || id == 17152 || id == 17151 || id == 17150 || id == 17149)
			n = new Legio(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
			...
			n = new Airut(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, spawned);
		else if (id == 13447)
			...
			n = new TormentedDemon(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, 
					spawned);
		else if (id == 15149)
			n = new MasterOfFear(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, 
					spawned);
		else if (id == 14696)
			n = new GanodermicBeast(id, tile, mapAreaNameHash, canBeAttackFromOutOfArea, 
					spawned);
		else 
				...
				public void run() {
					try {
						if (!region.forceGetFloorItems().contains(floorItem))
							return;
						int regionId = tile.getRegionId();
						if (underGrave || !ItemConstants.isTradeable(floorItem) || item.getName().contains("corey")) {
...
return true;
	...
	public static final boolean isMultiArea(WorldTile tile) {
		...
		return  (destX >= 3462 && destX <= 3511 && destY >= 9481 && destY <= 9521 && tile.getPlane() == 0) //kalphite queen lair
				...
				|| (destX >= 3535 && destX <= 3564 && destY >= 9486 && destY <= 9516)) //Vorago Cave
	...
	public static final boolean isAscensionArea(WorldTile tile) {
		int destX = tile.getX();
		int destY = tile.getY();
		return ((destX <= 6310 && destX >= 6299 && destY <= 6279 && destY >= 6291));
	}
	...
	public static boolean inPvPMode;
	
	public static void setPvpMode(boolean inPvPMode) {
		World.inPvPMode = inPvPMode;
	}
}
