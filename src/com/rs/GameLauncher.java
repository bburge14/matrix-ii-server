...
public class GameLauncher {
	...
	public static void main(String[] args) throws Exception {
		...
		com.rs.admin.AdminHttpServer.start();
			...
			Runtime.getRuntime().addShutdownHook(new Thread("Server-shutdown-save") {
				...
				public void run() {
					try {
						System.out.println("[Shutdown] === Beginning shutdown save ===");

						// 1. Save bots (direct disk write, no queue)
						try {
							System.out.println("[Shutdown] Saving online bots...");
							int nb = com.rs.bot.BotPool.saveOnline();
							System.out.println("[Shutdown] Bots saved: " + nb);
						} catch (Throwable t) {
							System.err.println("[Shutdown] Bot save failed: " + t);
						}

						// 2. Queue up saves for all online real players (sends via login server)
						int playerCount = 0;
						try {
							for (com.rs.game.player.Player player : com.rs.game.World.getPlayers()) {
								if (player == null || !player.hasStarted() || player.hasFinished()) continue;
								if (player instanceof com.rs.bot.AIPlayer) continue; // bots already saved above
								byte[] data = com.rs.utils.SerializationUtilities.tryStoreObject(player);
								if (data == null || data.length <= 0) continue;
								com.rs.executor.PlayerHandlerThread.addSave(player.getUsername(), data);
								playerCount++;
							}
							System.out.println("[Shutdown] Queued " + playerCount + " player save(s)");
						} catch (Throwable t) {
							System.err.println("[Shutdown] Player queue failed: " + t);
						}

						// 3. Wait for the PlayerHandlerThread save queue to drain (max 10s)
						//    The save queue sends packets to the login server which writes to its own MiniFS.
						if (playerCount > 0) {
							System.out.println("[Shutdown] Waiting for save queue to drain...");
							long deadline = System.currentTimeMillis() + 10000L;
							while (System.currentTimeMillis() < deadline) {
								if (com.rs.executor.PlayerHandlerThread.getPendingSaveCount() == 0) break;
								try { Thread.sleep(100); } catch (InterruptedException ie) { break; }
							}
							int remaining = com.rs.executor.PlayerHandlerThread.getPendingSaveCount();
							if (remaining > 0) {
								System.err.println("[Shutdown] WARNING: " + remaining + " saves still pending after 10s timeout");
							} else {
								System.out.println("[Shutdown] Save queue drained.");
							}
						}

						// 4. Flush the game-server filesystem (clans, GE offers, bots)
						try {
							com.rs.utils.SerializableFilesManager.flush();
							System.out.println("[Shutdown] Game-server filesystem flushed.");
						} catch (Throwable t) {
							System.err.println("[Shutdown] Game flush failed: " + t);
						}

						System.out.println("[Shutdown] === Shutdown save complete ===");
					} catch (Throwable t) {
						System.err.println("[Shutdown] Catastrophic shutdown hook failure: " + t);
						t.printStackTrace();
					}
				}
			});
		...
		// console.start(); // Disabled - no stdin under nohup, was spamming NoSuchElementException
	...
	public static boolean initShutdown() {
		...
		try {
			...
			Logger.log("Launcher", "Despawned " + n + " bots before shutdown.");
