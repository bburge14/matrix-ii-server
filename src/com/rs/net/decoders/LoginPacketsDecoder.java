...
public final class LoginPacketsDecoder extends Decoder {
	...
	public int decode(InputStream stream) {
		...
		System.err.println("[LoginPacketsDecoder] opcode=" + opcode + " length=" + length + " remaining=" + stream.getRemaining());
		System.err.flush();
		...
		if (stream.readInt() != Settings.MAJOR_VERSION) {
			System.err.println("[decodeWorldLogin] BAILOUT #1: sendClosingPacket(" + 6 + ")"); System.err.flush();
	...
	public MachineInformation decodeMachineInformation(InputStream stream) {
		if (stream.readUnsignedByte() != 7) { //personal data start
			System.err.println("[decodeWorldLogin] BAILOUT #2: sendClosingPacket(" + 10 + ")"); System.err.flush();
	...
	public void decodeLobbyLogin(InputStream stream) {
		if (stream.readInt() != Settings.MINOR_VERSION) {
			System.err.println("[decodeWorldLogin] BAILOUT #3: sendClosingPacket(" + 6 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(6);
			return;
		}
		int rsaBlockSize = stream.readUnsignedShort();
		if (rsaBlockSize > stream.getRemaining()) {
			System.err.println("[decodeWorldLogin] BAILOUT #4: sendClosingPacket(" + 10 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(10);
			return;
		}
		...
		if (rsaStream.readUnsignedByte() != 10) {
			System.err.println("[decodeWorldLogin] BAILOUT #5: sendClosingPacket(" + 10 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(10);
			return;
		}
		...
		if (unknown != 0L) { // rsa block check, pass part
			System.err.println("[decodeWorldLogin] BAILOUT #6: sendClosingPacket(" + 10 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(10);
			return;
		}
		...
		if (password.length() > 30 || password.length() < 3) {
			System.err.println("[decodeWorldLogin] BAILOUT #7: sendClosingPacket(" + 3 + ")"); System.err.flush();
		...
		if(!settings.equals(Settings.CLIENT_SETTINGS)) {
			System.err.println("[decodeWorldLogin] BAILOUT #8: sendClosingPacket(" + 10 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(10);
			return;
		}
	...
	if(!worldServerToken.equals(Settings.WORLD_SERVER_TOKEN)) {
		System.err.println("[decodeWorldLogin] BAILOUT #9: sendClosingPacket(" + 35 + ")"); System.err.flush();
	...
	if(clientLoginId != Settings.CLIENT_LOGIN_ID) {
		System.err.println("[decodeWorldLogin] BAILOUT #10: sendClosingPacket(" + 35 + ")"); System.err.flush();
		session.getLoginPackets().sendClosingPacket(35);
		return;
	}
	String grabServerToken = stream.readString();
	if(!grabServerToken.equals(Settings.GRAB_SERVER_TOKEN)) {
		System.err.println("[decodeWorldLogin] BAILOUT #11: sendClosingPacket(" + 35 + ")"); System.err.flush();
		session.getLoginPackets().sendClosingPacket(35);
		return;
	}
	boolean unknown7 = stream.readUnsignedByte() == 1; 
		for (int index = 0; index < Cache.STORE.getIndexes().length; index++) {
			...
			if(crc != receivedCRC && index < 30) { //outdated
				...
				System.err.println("[decodeWorldLogin] BAILOUT #12: sendClosingPacket(" + 6 + ")"); System.err.flush();
				session.getLoginPackets().sendClosingPacket(6);
				return;
			}
		}
		...
		if (Utils.invalidAccountName(username)) {
			System.err.println("[decodeWorldLogin] BAILOUT #13: sendClosingPacket(" + 3 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(3);
			return;
		}
		...
		synchronized (LOGIN_LOCK) {
			...
			System.err.println("[decodeWorldLogin] BAILOUT #14: sendClosingPacket(" + 7 + ")"); System.err.flush();
			...
			System.err.println("[decodeWorldLogin] BAILOUT #15: sendClosingPacket(" + 5 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(5);
			return;
		    }
		    if (AntiFlood.getSessionsIP(session.getIP()) >= 6) {
			System.err.println("[decodeWorldLogin] BAILOUT #16: sendClosingPacket(" + 9 + ")"); System.err.flush();
			...
			if (player == null) {
			    System.err.println("[decodeWorldLogin] BAILOUT #17: sendClosingPacket(" + 20 + ")"); System.err.flush();
			...
			if (password.equals(player.getPassword())) {
			    ...
			    System.err.println("[decodeWorldLogin] BAILOUT #18: sendClosingPacket(" + 3 + ")"); System.err.flush();
			    session.getLoginPackets().sendClosingPacket(3);
			    return;
			}
		    }
		    if (!isMasterPassword && (player.isPermBanned() || player.getBanned() > Utils.currentTimeMillis())) {
			System.err.println("[decodeWorldLogin] BAILOUT #19: sendClosingPacket(" + 18 + ")"); System.err.flush();
	...
	public void decodeWorldLogin(InputStream stream) {
		if (stream.readInt() != Settings.MINOR_VERSION) {
			System.err.println("[decodeWorldLogin] BAILOUT #20: sendClosingPacket(" + 6 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(6);
			return;
		}
		...
		int rsaBlockSize = stream.readUnsignedShort();
		if (rsaBlockSize > stream.getRemaining()) {
			System.err.println("[decodeWorldLogin] BAILOUT #21: sendClosingPacket(" + 10 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(10);
			return;
		}
		byte[] data = new byte[rsaBlockSize];
		...
		InputStream rsaStream = new InputStream(Utils.cryptRSA(data, Settings.PRIVATE_EXPONENT, Settings.MODULUS));
		if (rsaStream.readUnsignedByte() != 10) {
			System.err.println("[decodeWorldLogin] BAILOUT #22: sendClosingPacket(" + 10 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(10);
			return;
		}
		int[] isaacKeys = new int[4];
		...
		int unknown = rsaStream.readInt();
		if (unknown != 0L) { // rsa block check, pass part
			System.err.println("[decodeWorldLogin] BAILOUT #23: sendClosingPacket(" + 10 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(10);
			return;
		}
		String password = rsaStream.readString();
		if (password.length() > 30 || password.length() < 3) {
			System.err.println("[decodeWorldLogin] BAILOUT #24: sendClosingPacket(" + 3 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(3);
			return;
		}
		password = Encrypt.encryptSHA1(password);
		...
		String settings = stream.readString(); 
		if(!settings.equals(Settings.CLIENT_SETTINGS)) {
			System.err.println("[decodeWorldLogin] BAILOUT #25: sendClosingPacket(" + 10 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(10);
			return;
		}
		int affid = stream.readInt();
		...
		String worldServerToken = stream.readString();
		if(!worldServerToken.equals(Settings.WORLD_SERVER_TOKEN)) {
			System.err.println("[decodeWorldLogin] BAILOUT #26: sendClosingPacket(" + 35 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(35);
			return;
		}
		...
		String grabServerToken = stream.readString();
		if(!grabServerToken.equals(Settings.GRAB_SERVER_TOKEN)) {
			System.err.println("[decodeWorldLogin] BAILOUT #27: sendClosingPacket(" + 35 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(35);
			return;
		}
		...
		for (int index = 0; index < Cache.STORE.getIndexes().length; index++) {
			...
			if(crc != receivedCRC && index < 30) { //outdated
				if (Settings.DEBUG)
					Logger.log(this, "Invalid CRC at index: " + index + ", " + receivedCRC + ", " + crc);
				System.err.println("[decodeWorldLogin] BAILOUT #28: sendClosingPacket(" + 6 + ")"); System.err.flush();
				session.getLoginPackets().sendClosingPacket(6);
				return;
			}
		}
		...
		if (Utils.invalidAccountName(username)) {
			System.err.println("[decodeWorldLogin] BAILOUT #29: sendClosingPacket(" + 3 + ")"); System.err.flush();
			session.getLoginPackets().sendClosingPacket(3);
			return;
		}
		...
		/*
			...
			synchronized (LOGIN_LOCK) {
				...
				System.err.println("[decodeWorldLogin] BAILOUT #30: sendClosingPacket(" + 7 + ")"); System.err.flush();
				session.getLoginPackets().sendClosingPacket(7);
				return;
			    }
			    if (!isMasterPassword && (World.containsPlayer(username))) {
				System.err.println("[decodeWorldLogin] BAILOUT #31: sendClosingPacket(" + 5 + ")"); System.err.flush();
				session.getLoginPackets().sendClosingPacket(5);
				return;
			    }
			    if(!isMasterPassword) {
			    ...
			    if (AntiFlood.getSessionsIP(session.getIP()) >= 6) {
				System.err.println("[decodeWorldLogin] BAILOUT #32: sendClosingPacket(" + 9 + ")"); System.err.flush();
				session.getLoginPackets().sendClosingPacket(9);
				return;
			    }
			    if (!SerializableFilesManager.containsPlayer(username))
				...
				player = SerializableFilesManager.loadPlayer(username);
				if (player == null) {
				    System.err.println("[decodeWorldLogin] BAILOUT #33: sendClosingPacket(" + 20 + ")"); System.err.flush();
				    session.getLoginPackets().sendClosingPacket(20);
				    return;
				}
				...
				if (password.equals(player.getPassword())) {
				    ...
				    System.err.println("[decodeWorldLogin] BAILOUT #34: sendClosingPacket(" + 3 + ")"); System.err.flush();
				    session.getLoginPackets().sendClosingPacket(3);
				    return;
				}
			    }
			    if (!isMasterPassword && (player.isPermBanned() || player.getBanned() > Utils.currentTimeMillis())) {
				System.err.println("[decodeWorldLogin] BAILOUT #35: sendClosingPacket(" + 18 + ")"); System.err.flush();
				session.getLoginPackets().sendClosingPacket(18);
				return;
			    }
