...
public class GanodermicCombat extends CombatScript {
	...
	public int attack(NPC npc, Entity target) {
		...
		if (attackStyle == 0 || attackStyle == 1) {
			...
			delayHit(npc, 1,target, getMagicHit(npc,getMaxHit(npc, 650,NPCCombatDefinitions.MAGE, target)));
		...
		if (attackStyle == 2) {//fungal attack
			...
			delayHit(npc, 1, target, getMeleeHit(npc, getMaxHit(npc, 550,NPCCombatDefinitions.MELEE, target)));
		}
		return npc.getAttackSpeed();
	}
