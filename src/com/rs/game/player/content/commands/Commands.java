...
public final class Commands {
	...
	if (clientCommand) {
	    ...
	    case "auditmethods": {
		// Manually trigger an audit. Same logic as the auto-scheduled run
		// from GameLauncher; results go to data/logs/audit.log.
		com.rs.bot.BotAuditor.runAudit();
		player.getPackets().sendGameMessage("Audit done - see data/logs/audit.log");
		return true;
	    }
	...
	if (clientCommand) {
	    ...
	    case "admin":
		if (player.getRights() < 2) {
		    player.getPackets().sendGameMessage("You don't have permission.");
		    return true;
		}
		com.rs.game.player.admin.AdminPanel.open(player);
		return true;
	    case "admind":
		if (player.getRights() < 2) {
		    player.getPackets().sendGameMessage("You don't have permission.");
		...
		try {
			...
			int e = Integer.parseInt(cmd[2]);
			com.rs.game.player.admin.AdminPanelProbe.open(player, s, e);
		    } else {
			com.rs.game.player.admin.AdminPanelProbe.open(player);
		    }
		} catch (NumberFormatException nfe) {
		    player.getPackets().sendGameMessage("Usage: ::adminprobe [start] [end]");
		}
		return true;
	    case "listprobe":
		if (player.getRights() < 2) {
		...
		try {
			...
			int e = Integer.parseInt(cmd[3]);
			com.rs.game.player.admin.ListPanelProbe.open(player, iface, s, e);
		    } else if (cmd.length >= 2) {
			int iface = Integer.parseInt(cmd[1]);
			com.rs.game.player.admin.ListPanelProbe.open(player, iface);
		    } else {
			com.rs.game.player.admin.ListPanelProbe.open(player);
		    }
		} catch (NumberFormatException nfe) {
		    player.getPackets().sendGameMessage("Usage: ::listprobe [iface] [start] [end]");
		}
		return true;
	    case "liststep":
		if (player.getRights() < 2) {
		...
		try {
		    ...
		    int s = Integer.parseInt(cmd[2]);
		    int e = Integer.parseInt(cmd[3]);
		    int step = Integer.parseInt(cmd[4]);
		    com.rs.game.player.admin.ListPanelProbe.openStep(player, iface, s, e, step);
		} catch (Exception ex) {
		    player.getPackets().sendGameMessage("Usage: ::liststep <iface> <start> <end> <step>");
		}
		return true;
	    case "players":
		...
		return true;
	    case "help":
		player.getDialogueManager().startDialogue("HelpPanelD");
		return true;
	    case "wiki":
		player.getDialogueManager().startDialogue("HelpPanelD");
		return true;
		...
		return true;
	    case "itemdb":
		player.getPackets().sendGameMessage("The item database is not available on this server.");
		return true;
	    case "commands":
		player.getDialogueManager().startDialogue("HelpPanelD");
		return true;
	    case "itemlist":
		player.getPackets().sendGameMessage("The item list is not available on this server.");
		return true;
	    case "website":
		player.getPackets().sendGameMessage("No website configured.");
		return true;
	    case "livechat":
		player.getPackets().sendOpenURL("http://localhost/site/index.php?page=donate_other");
		return true;
	    case "reportplayer":
		player.getPackets().sendOpenURL("http://localhost/forms.php?do=form&fid=4");
		return true;
	    case "reportbug":
		player.getPackets().sendOpenURL("http://localhost/misc.php?do=form&fid=3");
		return true;
	    case "recoverpass":
		player.getPackets().sendOpenURL("http://localhost/misc.php?do=form&fid=1");
		return true;
	    case "appealoffence":
		player.getPackets().sendOpenURL("http://localhost/misc.php?do=form&fid=2");
		return true;
	    case "checkreport":
		player.getPackets().sendOpenURL("http://localhost/forumdisplay.php?153-Player-reports");
		return true;
	    case "checkappeal":
		player.getPackets().sendOpenURL("http://localhost/forumdisplay.php?142-Offence-Appeal");
		return true;
	    case "rules":
		player.getPackets().sendGameMessage("Rules are currently unavailable.");
		return true;
	    case "guides":
		player.getDialogueManager().startDialogue("HelpPanelD");
		return true;
		...
		player.getPackets()
			.sendOpenURL("http://localhost/showthread.php?" + Integer.parseInt(cmd[1]));
		return true;
	    case "yell":
		message = "";
		for (int i = 1; i < cmd.length; i++)
