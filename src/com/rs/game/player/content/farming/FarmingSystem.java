...
public class FarmingSystem {
	...
	public static int[] farmingPatches = { 8552, 8553, 7848, 8151, 8550, 8551, 7847, 8150, 8556 };
		...
		for (Seeds.Seed seed : Seeds.Seed.values()) {
			if (seedId == seed.getItem().getId()) {
				...
				if (!player.getInventory().containsItem(seed.getItem().getId(), seed.getItem().getAmount())) {
					player.getPackets().sendGameMessage("You need at least "+seed.getItem().getAmount()+" "+com.rs.cache.loaders.ItemDefinitions.getItemDefinitions(seedId).getName()+"'s.");
					return;
				}
				for (int i = 0; i < seed.getSuitablePatch().length; i++) {
					if (seed.getSuitablePatch()[i] == object.getId()) {
						player.getPackets().sendGameMessage("You plant some "+com.rs.cache.loaders.ItemDefinitions.getItemDefinitions(seedId).getName()+"'s.");
						player.getInventory().deleteItem(seed.getItem());
						player.setNextAnimation(new Animation(2291));
						player.farmingPatch.add(new PatchStatus(object.getId(), object.getDefinitions().configFileId, seed.getConfigValues()[0], seed.getConfigValues()[4], "Some "+com.rs.cache.loaders.ItemDefinitions.getItemDefinitions(seedId).getName()+"'s have been planted here."));
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if ((status.getConfigValue() + 1) == status.getMaxConfigValue()) {
					player.getVarsManager().sendVarBit(status.getConfigId(), status.getMaxConfigValue());
					...
					player.getVarsManager().sendVarBit(status.getConfigId(), status.getConfigValue() + 1);
				}
			}
		}, 0, time);
	}
	...
	private static void harvestCrops(Player player, WorldObject object) {
		if (canHarvest(player, object)) {
			player.getVarsManager().sendVarBit(object.getDefinitions().configFileId, 0);
			...
			if (patch.getObjectId() == object.getId()) {
				for (Seeds.Seed seed : Seed.values()) {
					if (seed.getConfigValues()[4] == patch.getMaxConfigValue()) {
						...
						player.getPackets().sendGameMessage("You harvest the "+com.rs.cache.loaders.ItemDefinitions.getItemDefinitions(seed.getProduce().getId()).getName()+"'s.");
					}
				}
			}
		}
	...
	public static void continueGrowth(final Player player) {
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				for (PatchStatus patch : player.farmingPatch) {
					if ((patch.getConfigValue() + 1) == patch.getMaxConfigValue()) {
						player.getVarsManager().sendVarBit(patch.getConfigId(), patch.getMaxConfigValue());
						...
						player.getVarsManager().sendVarBit(patch.getConfigId(), patch.getConfigValue() + 1);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				...
				if (loop == 0 || loop == 3 || loop == 6) {	
					...
					player.getVarsManager().sendVarBit(object.getDefinitions().configFileId, configValue);
